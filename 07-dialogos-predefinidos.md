...
> 💡 **Tip del Examinador**: Pregunta frecuente: "¿Qué diferencia hay entre Show() y ShowDialog()? - Respuesta: Show() es no modal (no bloquea), ShowDialog() es modal (bloquea hasta cerrar). Otra pregunta común: "¿Cómo obtener el resultado de un diálogo?" → Usando DialogResult = true/false y verificando el retorno de ShowDialog().
