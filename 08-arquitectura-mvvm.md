...
- [8.4. CommunityToolkit.Mvvm: La Biblioteca Oficial](#84-communitytoolkitmvvm-la-biblioteca-oficial)
  ...
  - [8.4.4. Reactividad en Modelos: El Problema de ObservableCollection](#844-reactividad-en-modelos-el-problema-de-observablecollection)
...
### 8.4.4. Reactividad en Modelos: El Problema de ObservableCollection

**El problema**: Cuando usamos `ObservableCollection<T>` en el ViewModel, la colección notifica a la UI cuando se **añaden o eliminan elementos**, pero **NO cuando cambian las propiedades internas** de los objetos que ya están en la lista.
...
2. Esto es ineficiente y puede causar parpadeos o pérdida de foco en la UI
...
sequenceDiagram
    ...
    participant Collection as ObservableCollection<Producto>
    
    User->>View: Marca checkbox
    View->>VM: Cambia EstaComprado = true
    VM->>Collection: productos[i] = new Producto(...)
    Collection->>View: NotifyCollectionChanged (añadir)
    View->>User: Se actualiza la lista (redibujar todo el item)
    
    Note over VM,Collection: ❌ Problemático: reemplazar objeto completo
    Note over View: ❌ Puede perder foco o parpadear
```

**La solución**: Usar `ObservableObject` + `[ObservableProperty]` en el Modelo
...
graph TB
    subgraph "SOLUCIÓN: Modelo hereda de ObservableObject"
        ...
        M --> M2["[ObservableProperty]<br/>[NotifyPropertyChangedFor(nameof(Total))]<br/>private int _cantidad"]
        
        M1 -->|"genera"| P1["public bool EstaComprado<br/>+ OnPropertyChanged"]
        M2 -->|"genera"| P2["public int Cantidad<br/>+ OnPropertyChanged"]
        
        P1 -->|"notifica"| V["Vista: TextBlock tachado se actualiza"]
...
sequenceDiagram
    ...
    View->>User: TextBlock del nombre se tachasó (solo ese elemento)
    
    Note over View: ✅ Solo se redibuja el elemento específico
    Note over View: ✅ No hay parpadeo ni pérdida de foco
    Note over View: ✅ Rendimiento óptimo
```

**Ventajas de usar ObservableObject en el Modelo**:

| Ventaja | Descripción |
|---------|-------------|
| **Actualización instantánea** | La propiedad cambia y la UI se actualiza inmediatamente |
| **Rendimiento óptimo** | Solo se redibuja el elemento específico, no toda la lista |
| **Código limpio** | CommunityToolkit genera automáticamente el código de notificación |
| **Propiedades calculadas** | Con `[NotifyPropertyChangedFor]`, si cambia cantidad, Total se actualiza automáticamente |

**Resumen**:

- `ObservableCollection<T>` → notifica cambios en la **colección** (add/remove)
- `ObservableObject` + `[ObservableProperty]` → notifica cambios en las **propiedades del objeto**
- Para listas con objetos que mutan → **ambos** necesarios

> 📝 **Nota del Profesor**: En el ejemplo de la Lista de la Compra (proyecto 14), el modelo Producto hereda de ObservableObject. Así, cuando el usuario marca el checkbox, la propiedad EstaComprado notifica el cambio directamente al TextBlock del nombre, aplicando el tachado de forma inmediata sin recargar la lista.
