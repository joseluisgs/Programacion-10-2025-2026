...
### 10.13.2. Show (No Modal) con Manejo de Eventos
...
### 10.13.3. Paso de Datos mediante Constructor
...
| Concepto | Descripción |
|----------|-------------|
| **Show()** | Ventana no modal - no bloquea la ventana padre |
| **ShowDialog()** | Ventana modal - bloquea hasta que se cierre |
| **DialogResult** | Resultado del diálogo (true/false/null) |
| **Owner** | Establece relación padre-hijo entre ventanas |
| **WindowStartupLocation** | Posición inicial: Manual, CenterScreen, CenterOwner |
| **Ciclo de vida** | Constructor → Initialized → Loaded → Activated → Closing → Closed |

### Puntos clave

1. **Show vs ShowDialog**: Show() es no bloqueante, ShowDialog() espera resultado.
2. **DialogResult**: Se usa con ShowDialog() para obtener respuesta del usuario.
3. **Owner**: Establece ventana padre, permite CenterOwner y cierre automático.
4. **Pasar datos**: Constructor, propiedades públicas, o DataContext (MVVM).
5. **Retornar datos**: Propiedades públicas, eventos, o callbacks.
6. **Ciclo de vida**: Loaded para inicialización, Closing para validación, Closed para cleanup.

> 📝 **Nota del Profesor**: La gestión de ventanas es más compleja que en web. Aprende bien la diferencia entre Show() y ShowDialog(), y cómo pasar datos entre ventanas. Es básico para cualquier app de escritorio. Recuerda: ShowDialog() devuelve bool? (nullable), compara siempre con == true.

> 💡 **Tip del Examinador**: Pregunta frecuente: "¿Cómo pasar datos de una ventana secundaria a la principal?"答: Usa propiedades públicas con ShowDialog() y verifica el DialogResult. También puedes usar eventos para un acoplamiento más débil. En el examen pueden preguntarte sobre el ciclo de vida: ¿qué evento es cancelable? → Closing. ¿cuál se ejecuta después de InitializeComponent? → Loaded.
