...
- [10.1. Introducción a la Gestión de Ventanas](#101-introducción-a-la-gestión-de-ventanas)
  - [10.1.1. Tipos de Ventanas](#1011-tipos-de-ventanas)
- [10.2. Show() vs ShowDialog()](#102-show-vs-showdialog)
  - [10.2.1. Show(): Ventana No Modal](#1021-show-ventana-no-modal)
  - [10.2.2. ShowDialog(): Ventana Modal](#1022-showdialog-ventana-modal)
  - [10.2.3. Tabla Comparativa](#1023-tabla-comparativa)
- [10.3. DialogResult: Retornar Resultados](#103-dialogresult-retornar-resultados)
  - [10.3.1. Establecer DialogResult](#1031-establecer-dialogresult)
  - [10.3.2. Uso del Diálogo](#1032-uso-del-diálogo)
- [10.4. Pasar Datos a Ventanas](#104-pasar-datos-a-ventanas)
  - [10.4.1. Mediante Constructor](#1041-mediante-constructor)
  - [10.4.2. Mediante Propiedades Públicas](#1042-mediante-propiedades-públicas)
  - [10.4.3. Mediante DataContext (Recomendado MVVM)](#1043-mediante-datacontext-recomendado-mvvm)
- [10.5. Retornar Datos desde Ventanas](#105-retornar-datos-desde-ventanas)
  - [10.5.1. Mediante Propiedades Públicas](#1051-mediante-propiedades-públicas)
  - [10.5.2. Mediante Eventos](#1052-mediante-eventos)
  - [10.5.3. Mediante Callback](#1053-mediante-callback)
- [10.6. Ciclo de Vida de una Ventana](#106-ciclo-de-vida-de-una-ventana)
  - [10.6.1. Eventos Principales](#1061-eventos-principales)
  - [10.6.2. Diagrama de Ciclo de Vida](#1062-diagrama-de-ciclo-de-vida)
- [10.7. Propiedades de Ventana](#107-propiedades-de-ventana)
  - [10.7.1. Propiedades Básicas](#1071-propiedades-básicas)
  - [10.7.2. WindowStartupLocation](#1072-windowstartuplocation)
  - [10.7.3. ResizeMode](#1073-resizemode)
- [10.8. Gestión de Múltiples Ventanas](#108-gestión-de-múltiples-ventanas)
  - [10.8.1. Application.Windows Collection](#1081-applicationwindows-collection)
  - [10.8.2. Owner y Owned Windows](#1082-owner-y-owned-windows)
  - [10.8.3. Singleton para Ventanas Únicas](#1083-singleton-para-ventanas-únicas)
- [10.9. Ejemplo Completo: Aplicación Multi-Ventana](#109-ejemplo-completo-aplicación-multi-ventana)
  - [10.9.1. Ventana Principal](#1091-ventana-principal)
  - [10.9.2. Diálogo de Nuevo Usuario](#1092-diálogo-de-nuevo-usuario)
- [10.10. Tips y Buenas Prácticas](#110-tips-y-buenas-prácticas)
  - [10.10.1. Evitar Memory Leaks](#1101-evitar-memory-leaks)
  - [10.10.2. Usar Owner Correctamente](#1102-usar-owner-correctamente)
  - [10.10.3. Validar antes de Cerrar](#1103-validar-antes-de-cerrar)
- [10.11. Frame, Page y Window](#111-frame-page-y-window)
  - [10.11.1. Window](#1111-window)
  - [10.11.2. Frame](#1112-frame)
  - [10.11.3. Page](#1113-page)
  - [10.11.4. Resumen](#1114-resumen)
- [10.12. Sistema de Navegación con Frame](#112-sistema-de-navegación-con-frame)
- [10.13. ShowDialog() para Diálogos](#113-showdialog-para-diálogos)
- [10.14. Paso de Parámetros entre Vistas](#114-paso-de-parámetros-entre-vistas)
- [10.15. Referencias](#115-referencias)

## 10.1. Introducción a la Gestión de Ventanas
...
> 📝 **Nota del Profesor**: La gestión de ventanas es más compleja que en web. Aprende bien la diferencia entre Show() y ShowDialog(), y cómo pasar datos entre ventanas. Es básico para cualquier app de escritorio.

### 10.1.1. Tipos de Ventanas
...
## 10.2. Show() vs ShowDialog()

### 10.2.1. Show(): Ventana No Modal
...
### 10.2.2. ShowDialog(): Ventana Modal
...
## 10.3. DialogResult: Retornar Resultados

### 10.3.1. Establecer DialogResult
...
## 10.4. Pasar Datos a Ventanas

### 10.4.1. Mediante Constructor
...
### 10.4.2. Mediante Propiedades Públicas
...
### 10.4.3. Mediante DataContext (Recomendado MVVM)
...
## 10.5. Retornar Datos desde Ventanas

### 10.5.1. Mediante Propiedades Públicas
...
> 💡 **Tip del Examinador**: En el examen, si te preguntan cómo pasar datos de una ventana secundaria a la principal, la respuesta es usar **propiedades públicas** con `ShowDialog()` y verificar el `DialogResult`. También puedes usar **eventos** para un acoplamiento más débil.

### 10.5.2. Mediante Eventos
...
## 10.6. Ciclo de Vida de una Ventana

### 10.6.1. Eventos Principales
...
### 10.6.2. Diagrama de Ciclo de Vida
...
## 10.7. Propiedades de Ventana

### 10.7.1. Propiedades Básicas
...
## 10.8. Gestión de Múltiples Ventanas

### 10.8.1. Application.Windows Collection
...
### 10.8.3. Singleton para Ventanas Únicas
...
## 10.9. Ejemplo Completo: Aplicación Multi-Ventana

### 10.9.1. Ventana Principal
...
### 10.9.2. Diálogo de Nuevo Usuario
...
### 10.10.2. Usar Owner Correctamente
...
### 10.10.3. Validar antes de Cerrar
...
## 10.11. Frame, Page y Window: Diferencias

En WPF existen 3 contenedores principales para mostrar contenido:

| Elemento | Tipo | Uso | Navegación |
|---------|------|-----|-----------|
| **Window** | `Window` | Ventana independiente (raíz app) | `Show()` / `ShowDialog()` |
| **Page** | `Page` | Contenido dentro de Frame | `Frame.Navigate(page)` |
| **UserControl** | `UserControl` | Componente reutilizable | Se usa como contenido |

### 10.11.1. Window (Ventana completa)

```csharp
var window = new MainWindow();
window.Show();        // No modal
window.ShowDialog(); // Modal - bloquea padre
```

- Es una **ventana completa** con barra de título, bordes, botones min/max/close
- Puede existir independientemente como ventana raíz
- Puede tener otras ventanas como "owned windows"
...
- **Contenedor de páginas** con historial de navegación
- Permite GoBack() / GoForward()
- Se usa para navegación tipo "web" en desktop

### 10.11.3. Page (Página)

```xml
<Page x:Class="EstudiantesPage">
    <Grid>...</Grid>
</Page>
```

- **Contenido** que se carga dentro de un Frame
- No tiene barra de título (solo contenido)
- Navegación: `Frame.Navigate(new Page())`

### 10.11.4. Resumen

| Característica | Window | Page (en Frame) |
|----------------|--------|-----------------|
| Historia navegación | No | Sí (back/forward) |
| Título y bordes | Sí | No (solo contenido) |
| Uso típico | Ventanas principales | Secciones de app |

---

## 10.12. Sistema de Navegación con Frame
...
[RelayCommand]
private void NavigateToEstudiantes() => OnNavigateRequested?.Invoke(new EstudiantesView());
```
...
**Flujo:** Command → evento → MainFrame.Navigate(page)

---

## 10.13. ShowDialog() para Diálogos Modales
...
## 10.14. Paso de Parámetros entre Vistas

**Constructor:** `new Ventana(dato)` → `ventana.Dato`
**DataContext:** `new Ventana { DataContext = vm }` → binding directo
**Propiedad:** `ventana.Propiedad = dato` → `ventana.Propiedad`
...
### 10.13.4. Paso de Datos mediante Propiedades
...
### 10.13.5. Paso de Datos con Eventos y Callbacks
...
### 10.13.6. ViewModel Compartido entre Ventanas
...
### 10.13.7. Patrones de DialogResult
...
### 10.13.8. Tabla Completa de Propiedades de Window
...
### 10.13.9. Diagrama de Flujo de Navegación entre Ventanas
...
flowchart TD
    ...
    style A fill:#4A90D9,stroke:#333,color:#000
    style E fill:#F5A623,stroke:#333,color:#000
    style L fill:#7ED321,stroke:#333,color:#000
```
