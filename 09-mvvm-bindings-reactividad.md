...
- [9.3. BINDINGS EN XAML](#93-bindings-en-xaml)
  ...
  - [9.3.13. partial void OnChanged - Reaccionar al cambio](#9313-parcial-void-onchanged---reaccionar-al-cambio)
  - [9.3.14. Collection Binding - ItemsSource](#9314-collection-binding---itemssource)
  - [9.3.15. IDataErrorInfo - Validación de Formularios](#9315-idataerrorinfo---validación-de-formularios)
  - [9.3.16. Diagrama: ciclo de vida de un binding](#9316-diagrama-ciclo-de-vida-de-un-binding)
- [9.4. IValueConverter](#94-ivalueconverter)
  - [9.4.1. Implementación básica](#941-implementación-básica)
  - [9.4.2. BooleanToVisibilityConverter](#942-booleantovisibilityconverter)
  - [9.4.3. Registro en recursos](#943-registro-en-recursos)
  - [9.4.4. Uso en binding](#944-uso-en-binding)
- [9.5. Converters: ¿Qué son y por qué usarlos?](#95-converters-qué-son-y-por-qué-usarlos)
  - [9.5.1. ¿Qué es un Converter en WPF?](#951-qué-es-un-converter-en-wpf)
  - [9.5.2. ¿Por qué necesitamos Converters?](#952-por-qué-necesitamos-converters)
  - [9.5.3. La interfaz IValueConverter](#953-la-interfaz-ivalueconverter)
  - [9.5.4. Ejemplo: InverseBooleanConverter](#954-ejemplo-inversebooleanconverter)
  - [9.5.5. Converter con parámetros](#955-converter-con-parámetros)
  - [9.5.6. Converters más comunes](#956-converters-más-comunes)
  - [9.5.7. Dónde registrar los Converters](#957-dónde-registrar-los-converters)
  - [9.5.8. Pros y Contras de usar Converters](#958-pros-y-contras-de-usar-converters)
  - [9.5.9. Resumen: Cuándo usar Converters](#959-resumen-cuándo-usar-converters)
- [9.6. ValidationRules](#96-validationrules)
  - [9.6.1. ValidationRule personalizada](#961-validationrule-personalizada)
  - [9.6.2. Uso en binding](#962-uso-en-binding)
  - [9.6.3. IDataErrorInfo](#963-idataerrorinfo)
- [9.7. TABLA COMPARATIVA](#97-tabla-comparativa)
  - [9.7.1. Diagrama: flujo de datos en MVVM](#971-diagrama-flujo-de-datos-en-mvvm)
...
### ⚠️ LIMITACIÓN IMPORTANTE: ObservableCollection no lo sabe todo

**El problema**: `ObservableCollection<T>` solo notifica a la UI cuando se **añaden o eliminan elementos**, pero **NO cuando cambian las propiedades internas** de los objetos que ya están en la lista.

```mermaid
graph LR
    subgraph "ObservableCollection - Solo notifica ADICIÓN/ELIMINACIÓN"
        A["productos.Add(p)"] --> B["UI se actualiza ✓"]
        C["productos.Remove(p)"] --> B
        D["producto.EstaComprado = true"] --> E["UI NO se actualiza ✗"]
    end
...
**Caso práctico - Lista de la compra**:
- Añadir producto → la UI se actualiza ✓
- Eliminar producto → la UI se actualiza ✓
- **Marcar checkbox (cambiar EstaComprado) → la UI NO se actualiza ✗**

**¿Por qué es un problema?**
1. Para actualizar la UI, tendríamos que reemplazar el objeto entero en la colección
2. Esto puede causar parpadeos o pérdida de foco en la UI
3. Si la lista es grande, recargar todos los elementos es costoso

### ✅ SOLUCIÓN: Modelo hereda de ObservableObject

La solución es que el **Modelo** herede de `ObservableObject` y use `[ObservableProperty]`:

```mermaid
graph TB
    subgraph "SOLUCIÓN: Modelo hereda de ObservableObject"
        M["Modelo: Producto<br/>hereda de ObservableObject"]
        
        M --> M1["[ObservableProperty]<br/>private bool _estaComprado"]
        M1 -->|"genera"| P1["public bool EstaComprado<br/>+ OnPropertyChanged"]
        
        P1 -->|"notifica"| V["Vista: TextBlock tachado"]
...
sequenceDiagram
    ...
    participant VM as ViewModel
    participant Model as Modelo (ObservableObject)
    
    User->>View: Marca checkbox
    View->>Model: producto.EstaComprado = true
    Model->>View: OnPropertyChanged("EstaComprado")
    View->>User: TextBlock del nombre se tacha (solo ese elemento)
    
    Note over View: ✅ Solo se redibuja el elemento específico
    Note over View: ✅ No hay parpadeo ni pérdida de foco
```

### Resumen: ObservableCollection vs ObservableObject

| Mecanismo | Notifica | Cuándo |
|-----------|----------|--------|
| `ObservableCollection<T>` | Cambios en la **colección** | Add, Remove, Clear |
| `ObservableObject` + `[ObservableProperty]` | Cambios en las **propiedades del objeto** | Cuando cambia una propiedad |

**Para listas con objetos que mutan → ambos necesarios**

> 📝 **Nota del Profesor**: En el ejemplo de la Lista de la Compra (proyecto 14), el modelo Producto hereda de ObservableObject. Así cuando el usuario marca el checkbox, la propiedad EstaComprado notifica directamente al TextBlock del nombre para aplicar el tachado.
...
### Ejemplo completo: Lista de la Compra con Reactividad

Este ejemplo muestra una lista donde:
- El modelo hereda de `ObservableObject`
- Al marcar checkbox, el nombre se tacha automáticamente
...
public partial class Producto : ObservableObject
{
    [ObservableProperty] private bool _estaComprado;
...
**ViewModel:**
```csharp
public partial class ListaCompraViewModel : ObservableObject
{
    [ObservableProperty]
    private ObservableCollection<Producto> _productos = new();
    ...
    [RelayCommand]
    private void MarcarComprado(Producto p) => p.EstaComprado = !p.EstaComprado;
}
```
...
**XAML - Binding con converter:**
```xml
<TextBlock TextDecorations="{Binding EstaComprado, 
           Converter={StaticResource BoolToTextDecoration}}"/>
```
...
> 📝 **Nota del Profesor**: Este ejemplo resume todo: modelo reactivo + ObservableCollection + converter + binding automático. Es el patrón estándar para listas en WPF.
...
sequenceDiagram
    participant User as Usuario
    participant View as Vista (XAML)
    participant VM as ViewModel
    participant Model as Modelo (Producto)
    
    User->>View: Marca checkbox
    View->>Model: producto.EstaComprado = true
    Model->>View: OnPropertyChanged("EstaComprado")
    View->>User: TextBlock nombre se tacha (solo ese elemento)
    
    Note over View: ✅ Solo redibuja el elemento específico
    Note over VM: ✅ OnPropertyChanged(nameof(Total)) actualiza el total
```

> 📝 **Nota del Profesor**: Este es el patrón correcto para listas reactivas en WPF con CommunityToolkit.Mvvm. El modelo debe heredar de ObservableObject para que las propiedades internas notifiquen cambios a la UI.
...
### 9.3.16. Diagrama: ciclo de vida de un binding
...
- Escribir código en el code-behind para cada control
...
### 9.6.1. ValidationRule personalizada
...
### 9.7.1. Diagrama: flujo de datos en MVVM
