...
## Por qué usar ObservableCollection

Este proyecto usa **ObservableCollection<T>** en lugar de **List<T>** para todas las colecciones de datos. Esto es **fundamental** en WPF MVVM:

```csharp
// ❌ INCORRECTO: List<T> NO notifica cambios a la UI
public List<Producto> Productos { get; set; } = new();

// ✅ CORRECTO: ObservableCollection<T> SÍ notifica
public ObservableCollection<Producto> Productos { get; set; } = new();
```

### ¿Por qué importa en este proyecto?

| Control | Binding | Sin ObservableCollection | Con ObservableCollection |
|---------|---------|-------------------------|-------------------------|
| **ListView** | ItemsSource | No se actualiza al filtrar | ✅ Se actualiza automáticamente |
| **DataGrid** | ItemsSource | No muestra cambios | ✅ Añadir/eliminar fila actualiza la UI |
| **ComboBox** | ItemsSource | No actualiza opciones | ✅ Cambios en lista se reflejan |

### Ejemplo: Filtrar productos por categoría

```csharp
// ViewModel - Cambio de categoría dispara filtro
partial void OnCategoriaSeleccionadaChanged(string value)
{
    if (string.IsNullOrEmpty(value) || value == "Todas")
    {
        // ✅ Si fuera List<T>, esto NO actualizaría el ListView
        ProductosFiltrados = new ObservableCollection<Producto>(Productos);
    }
    else
    {
        var filtrados = Productos.Where(p => p.Categoria == value);
        ProductosFiltrados = new ObservableCollection<Producto>(filtrados);
    }
}
```

> 💡 **El binding `{Binding ProductosFiltrados}` en el XAML se actualiza automáticamente** gracias a que usamos `ObservableCollection`.

### En este proyecto se usa en:

- `Productos` - Lista de productos original
- `ProductosFiltrados` - Lista filtrada por categoría (bindeada al ListView)
- `Estudiantes` - Lista de estudiantes (bindeada al DataGrid)
- `Categorias` - Lista de categorías para el ComboBox
...
> 💡 **Aceleradores de teclado**: Este proyecto demuestra el uso de:
> - `InputGestureText="Ctrl+N"` en XAML → Muestra el atajo junto al menú
> - `<KeyBinding Key="N" Modifiers="Control" Command="{Binding NuevoCommand}" />` → **Hace funcionar el atajo**
> - Al presionar **Ctrl+N** se ejecuta el comando `NuevoCommand` del ViewModel
>
> Ver teoría completa en [04-introduccion.md: 4.4.6. Menu y MenuItem](../../04-introduccion.md#446-menu-y-menuitem)
