...
public class PokedexRepository : IPokedexRepository
{
    ...
    private Dictionary<int, Pokemon> _pokemons = [];
    ...
    /// <inheritdoc/>
    public IEnumerable<Pokemon> GetAll() => _pokemons.Values;

    /// <inheritdoc/>
    public Pokemon? GetById(int id) => _pokemons.FirstOrDefault(p => p.Key == id).Value;
    ...
    public IEnumerable<Pokemon> GetByName(string name)
    {
        ...
        return _pokemons.Where(p => p.Value.Name.Contains(name, StringComparison.OrdinalIgnoreCase))
                        .Select(p => p.Value);
    }
    ...
    public IEnumerable<Pokemon> GetByType(string type)
    {
        ...
        return _pokemons.Where(p => p.Value.Type.Any(t => t.Equals(type, StringComparison.OrdinalIgnoreCase)))
                        .Select(p => p.Value);
    }
    ...
    public void Load(IEnumerable<Pokemon> pokemons)
    {
        ...
        _pokemons = pokemons.ToDictionary(p => p.Id);
