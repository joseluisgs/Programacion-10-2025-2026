...
public partial class GraficosView : Page {
    ...
    private void InitializeNotasChart() {
        try {
            ...
            var barData = calificaciones.Select((v, i) => new Bar {
                Position = i,
                Value = v,
                FillColor = Color.FromHex("#667EEA"),
                LineWidth = 0
            }).ToArray();
            ...
            NotasChart.Plot.Axes.Bottom.TickGenerator = new NumericManual(
                Enumerable.Range(0, ciclosLabels.Length)
                    .Select(i => new Tick(i, ciclosLabels[i]))
                    .ToArray()
            );
    ...
    private void InitializeDistributionChart() {
        try {
            ...
            var slices = new List<PieSlice> {
                new() {
                    Value = distribution[0], Label = $"Suspensos ({(int)distribution[0]})",
                    FillColor = Color.FromHex("#EF4444")
                },
                new() {
                    Value = distribution[1], Label = $"Aprobados ({(int)distribution[1]})",
                    FillColor = Color.FromHex("#F59E0B")
                },
                new() {
                    Value = distribution[2], Label = $"Notables ({(int)distribution[2]})",
                    FillColor = Color.FromHex("#3B82F6")
                },
                new() {
                    Value = distribution[3], Label = $"Sobresalientes ({(int)distribution[3]})",
                    FillColor = Color.FromHex("#10B981")
                }
            };
            ...
            DistributionChart.Plot.Legend.Alignment = Alignment.MiddleRight;
    ...
    private void InitializeCiclosChart() {
        try {
            ...
            var estudiantesBars = positions.Select((pos, i) => new Bar {
                Position = pos - 0.2,
            ...
            var docentesBars = positions.Select((pos, i) => {
                ...
                return new Bar {
                    ...
                    FillColor = Color.FromHex("#764BA2"),
                    Label = "Docentes"
                };
            }).ToArray();
            ...
            CiclosChart.Plot.Legend.Alignment = Alignment.UpperRight;
            CiclosChart.Plot.Legend.FontSize = 11;

            CiclosChart.Plot.Axes.Bottom.TickGenerator = new NumericManual(
                positions.Select((pos, i) => new Tick(pos, estudiantesLabels[i])).ToArray()
            );
    ...
    private void InitializeEdadChart() {
        try {
            ...
            var barData = edadValues.Select((v, i) => new Bar {
                Position = i,
                Value = v,
                FillColor = Color.FromHex("#28A745"),
                LineWidth = 0
            }).ToArray();
            ...
            EdadChart.Plot.Axes.Bottom.TickGenerator = new NumericManual(
                Enumerable.Range(0, edadLabels.Length)
                    .Select(i => new Tick(i, edadLabels[i]))
                    .ToArray()
            );
