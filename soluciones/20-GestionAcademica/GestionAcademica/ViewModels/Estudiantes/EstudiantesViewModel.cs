...
public partial class EstudiantesViewModel : ObservableObject
{
    ...
    private void FilterEstudiantes()
    {
        ...
        Estudiantes = new ObservableCollection<Estudiante>(pagina);
        
        if (CicloSeleccionado != "Todos" && !string.IsNullOrEmpty(CicloSeleccionado) && !string.IsNullOrWhiteSpace(SearchText))
            StatusMessage = $"Página {PaginaActual}/{TotalPaginas} - Mostrando {Estudiantes.Count} de {TotalRegistros} estudiantes";
        else if (CicloSeleccionado != "Todos" && !string.IsNullOrEmpty(CicloSeleccionado))
            StatusMessage = $"Página {PaginaActual}/{TotalPaginas} - {Estudiantes.Count} de {TotalRegistros} estudiantes en {CicloSeleccionado}";
