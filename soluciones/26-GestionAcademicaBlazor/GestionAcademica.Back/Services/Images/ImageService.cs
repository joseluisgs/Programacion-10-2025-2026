...
public class ImageService : IImageService {
    ...
    public ImageService(string imagesDirectory, string[] allowedExtensions) {
        ...
        if (!Directory.Exists(_imagesDirectory)) Directory.CreateDirectory(_imagesDirectory);
    }
    ...
    public async Task<Result<string, DomainError>> SaveImageAsync(string sourcePath) {
        ...
        var sizeValidation = await ValidateImageSizeAsync(sourcePath);
        ...
        var dimensionsValidation = await ValidateImageDimensionsAsync(sourcePath);
    ...
    public Task<Result<bool, DomainError>> DeleteImageAsync(string fileName) {
        _logger.Information("Eliminando imagen {FileName}", fileName);

        var path = Path.Combine(_imagesDirectory, fileName);

        if (!File.Exists(path))
            return Task.FromResult(Result.Failure<bool, DomainError>(ImageErrors.NotFound(fileName)));
        ...
        try {
            ...
            return Task.FromResult(Result.Failure<bool, DomainError>(ImageErrors.DeleteError(ex.Message)));
        }
    }

    public async Task<Result<bool, DomainError>> UpdateImageAsync(string sourcePath, string existingFileName) {
        _logger.Information("Actualizando imagen {ExistingFileName} con contenido de {SourcePath}", existingFileName,
            sourcePath);
    ...
    public async Task<bool> IsValidImageAsync(string sourcePath) {
        var extension = Path.GetExtension(sourcePath).ToLower();
        return await Task.FromResult(_allowedExtensions.Contains(extension));
    }

    public Task<Result<bool, DomainError>> ValidateImageSizeAsync(string sourcePath, long maxSizeInBytes = DefaultMaxSizeInBytes) {
    ...
    public Task<Result<bool, DomainError>> ValidateImageDimensionsAsync(string sourcePath, int maxWidth = DefaultMaxWidth,
        ...
        try {
            ...
            if (!allowedMimeTypes.Contains(mimeType))
                return Result.Failure<string, DomainError>(
                    ImageErrors.InvalidFormat(
                        $"Tipo MIME no permitido: {mimeType}. Solo se permiten PNG, JPEG, BMP y GIF."));
