...
public class BackupService(
    ...
    public async Task<Result<string, DomainError>> RealizarBackupAsync(
        ...
        try {
            ...
            return Result.Failure<string, DomainError>(
                BackupErrors.DirectoryError($"No se pudo crear el directorio: {backDirectory}"));
        }
        ...
        try {
            ...
            var jsonPath = Path.Combine(dataDir, AppConfig.PersonasFileName);
            var salvarResult = await storage.SalvarAsync(personasList, jsonPath);
    ...
    public async Task<Result<IEnumerable<Persona>, DomainError>> RestaurarBackupAsync(
        ...
        try {
            ...
            if (!File.Exists(jsonPath)) {
                _logger.Warning("El archivo de backup no contiene datos válidos ({FileName} no encontrado).", AppConfig.PersonasFileName);
                return Result.Failure<IEnumerable<Persona>, DomainError>(
                    BackupErrors.InvalidBackupFile("El archivo de backup no contiene datos válidos."));
            }

            var cargarResult = await storage.CargarAsync(jsonPath);
    ...
    public async Task<IEnumerable<string>> ListarBackupsAsync(string? customBackupDirectory = null) {
        var backDirectory = customBackupDirectory ?? _defaultBackupDirectory;
        if (backDirectory == null || !Directory.Exists(backDirectory))
            return Enumerable.Empty<string>();

        return await Task.FromResult(Directory.GetFiles(backDirectory, "*.zip")
            .OrderByDescending(f => File.GetCreationTime(f)));
    }

    public async Task<Result<string, DomainError>> RealizarBackupSistemaAsync(IEnumerable<Persona> personas) {
        return await RealizarBackupAsync(personas);
    }

    public async Task<Result<int, DomainError>> RestaurarBackupSistemaAsync(
        string archivoBackup,
        ...
        await CleanImagesDirectoryAsync(customImagesDirectory);

        // 3. Restaurar datos e imágenes del backup
        var restoreResult = await RestaurarBackupAsync(archivoBackup, customImagesDirectory);
        return restoreResult.Bind(personas => {
            var contador = 0;
    ...
    private async Task CleanImagesDirectoryAsync(string? customDirectory = null) {
        try {
            var imagesDir = customDirectory != null
                ? Path.Combine(customDirectory, AppConfig.BackupImagesDirectory)
                : AppConfig.ImagesDirectory;
    ...
    private static string GetImageSourcePath(string imageName) {
        return Path.Combine(AppConfig.ImagesDirectory, imageName);
    }
    ...
    private static string GetImageDestPath(string imageName, string? customDirectory = null) {
        var imagesDir = customDirectory != null
            ? Path.Combine(customDirectory, AppConfig.BackupImagesDirectory)
            : AppConfig.ImagesDirectory;
        return Path.Combine(imagesDir, imageName);
    }
