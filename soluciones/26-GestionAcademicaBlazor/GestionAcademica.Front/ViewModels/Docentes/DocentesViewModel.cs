...
public partial class DocentesViewModel : ObservableObject {
    ...
    private async Task LoadDocentesAsync() {
        ...
        try {
            var result = await _personasService.GetDocentesOrderByAsync(OrdenActual, 1, int.MaxValue, MostrarEliminados);
    ...
    private async Task DeleteAsync() {
        ...
        var deleteResult = await _personasService.DeleteAsync(SelectedDocente.Id, AppConfig.UseLogicalDelete);
    ...
    private async Task RestoreAsync() {
        ...
        if (!await _dialogService.ShowConfirmationAsync($"¿Restaurar a {SelectedDocente.NombreCompleto}?"))
            return;

        var result = await _personasService.RestoreAsync(SelectedDocente.Id);
