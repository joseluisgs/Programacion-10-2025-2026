# 🎓 Sistema de Gestión Académica (Blazor Server) - Guía Completa para Estudiantes

> **Un proyecto real de arquitectura en capas con Blazor Server, Clean Architecture y patrones de diseño profesionales**

![Badge Blazor](https://img.shields.io/badge/Blazor%20Server-.NET%2010-purple)
![Badge Clean Architecture](https://img.shields.io/badge/Architecture-Clean-orange)
![Badge Async](https://img.shields.io/badge/Async-Await-green)

---

## 📋 Tabla de Contenidos

- [🏗️ Arquitectura de Proyectos](#-arquitectura-de-proyectos)
...
- [2. Arquitectura del Sistema](#2-arquitectura-del-sistema)
  - [2.1. Visión General de la Arquitectura](#21-visión-general-de-la-arquitectura)
    - [2.1.1. Estructura de Proyectos (Back + Front + Test)](#211-estructura-de-proyectos-back--front--test)
    - [2.1.2. Diagrama de Capas del Sistema Completo](#212-diagrama-de-capas-del-sistema-completo)
      - [🎯 Responsabilidades de cada Capa](#-responsabilidades-de-cada-capa)
    - [2.1.3. Diagrama de Componentes (Frontend + Backend)](#213-diagrama-de-componentes-frontend--backend)
...
- [3. Capa de Datos (Backend)](#3-capa-de-datos-backend)
  ...
  - [3.3. Caché LRU](#33-caché-lru)
    ...
    - [3.3.3.1. Caché Dual (ID + DNI)](#3331-caché-dual-id--dni)
...
- [6. UI - Blazor Server](#6-ui---blazor-server)
  - [6.1. Arquitectura Blazor Server](#61-arquitectura-blazor-server)
    - [6.1.1. Flujo de la UI en Blazor Server](#611-flujo-de-la-ui-en-blazor-server)
    - [6.1.2. DI en Blazor](#612-di-en-blazor)
- [7. Componentes Blazor](#7-componentes-blazor)
- [8. Lógica de Componentes (Code-Behind)](#8-lógica-de-componentes-code-behind)
- [9. Arquitectura Blazor (Server)](#9-arquitectura-blazor-server)
  - [9.1. Circuito Blazor Server](#91-circuito-blazor-server)
  - [9.2. Gestión de Estado](#92-gestión-de-estado)
  - [9.3. Árbol de Componentes](#93-árbol-de-componentes)
  - [9.4. Navegación con NavigationManager](#94-navegación-con-navigationmanager)
  - [9.5. Estado y Renderizado](#95-estado-y-renderizado)
- [10. Servicios UI](#10-servicios-ui)
  - [10.1. DialogService (Blazor)](#101-dialogservice-blazor)
  - [10.2. ImageService](#102-imageservice)
  - [10.3. Ficheros de Recursos Blazor](#103-ficheros-de-recursos-blazor)
- [11. Binding y Eventos en Blazor](#11-binding-y-eventos-en-blazor)
  - [11.1. Data Binding en Blazor](#111-data-binding-en-blazor)
- [12. Búsqueda y Filtrado (Frontend)](#12-búsqueda-y-filtrado-frontend)
  - [12.1. Búsqueda en Tiempo Real](#121-búsqueda-en-tiempo-real)
  - [12.2. Ordenación Multicriterio](#122-ordenación-multicriterio)
- [13. Gestión de Estado Visual (Frontend)](#13-gestión-de-estado-visual-frontend)
  - [13.1. Estados de Carga](#131-estados-de-carga)
  - [13.2. Feedback Visual](#132-feedback-visual)
...
- [13. Principios y Patrones Aplicados](#13-principios-y-patrones-aplicados)
  ...
  - [13.3. Patrones de Diseño Implementados](#133-patrones-de-diseño-implementados)
    ...
    - [13.3.3. Blazor Component Model (separación UI/lógica)](#1333-blazor-component-model-separación-uilógica)
    ...
    - [13.3.6. Observer Pattern (EventCallback + StateHasChanged)](#1336-observer-pattern-eventcallback--statehaschanged)
...
- [14. Lo que Hemos Aprendido](#14-lo-que-hemos-aprendido)
  ...
  - [14.3. Validación Multicapa](#143-validación-multicapa)
    - [14.3.1. Validación en Frontend (FluentValidation/DataAnnotations) para UX](#1431-validación-en-frontend-fluentvalidationdataannotations-para-ux)
  ...
  - [14.4. Gestión de Estado en Blazor Server](#144-gestión-de-estado-en-blazor-server)
    - [14.4.1. StateHasChanged y reactividad](#1441-statehaschanged-y-reactividad)
    - [14.4.2. Colecciones dinámicas en Blazor](#1442-colecciones-dinámicas-en-blazor)
    - [14.4.3. Filtrado en Blazor con LINQ](#1443-filtrado-en-blazor-con-linq)
    - [14.4.4. Source Generators en Blazor](#1444-source-generators-en-blazor)
  - [14.5. Comunicación Desacoplada](#145-comunicación-desacoplada)
    - [14.5.1. Inyección de Dependencias y Event Callbacks en Blazor](#1451-inyección-de-dependencias-y-event-callbacks-en-blazor)
  ...
  - [14.6. Optimización con Caché](#146-optimización-con-caché)
    ...
    - [14.6.5. Caché Dual (búsquedas por ID y DNI)](#1465-caché-dual-búsquedas-por-id-y-dni)
  ...
  - [14.10. Testing y Mantenibilidad](#1410-testing-y-mantenibilidad)
    ...
    - [14.10.5. Testing de Componentes Gráficos (LiveCharts2)](#14105-testing-de-componentes-gráficos-livecharts2)
...
- ✅ **Interfaz profesional** (Blazor Server con SignalR)
...
| RNF-13 | **Arquitectura**     | El frontend (Blazor Server) y backend (Servicios) deben estar completamente desacoplados mediante interfaces.                               |
...
### 2.1.1. Estructura de Proyectos (Back + Front + Test)

Este proyecto utiliza una **separación en tres proyectos** dentro de la solución:

```
GestionAcademica.slnx
├── GestionAcademica.Back/        # Lógica de negocio, servicios, repositorios
├── GestionAcademica.Blazor/     # Blazor Server UI
├── GestionAcademica.Front/      # UI WPF con MVVM (legacy)
└── GestionAcademica.Test/       # Tests unitarios
```

**¿Por qué proyectos separados?**

| Beneficio | Descripción |
|-----------|-------------|
| **Portabilidad** | El Back contiene TODA la lógica de negocio independiente de la UI. Se puede reutilizar en Blazor, MAUI, Console, API, etc. |
| **Testabilidad** | La lógica de negocio se testa sin depender de la UI concreta. Back y Test compilan sin necesidad de entorno Windows. |
| **Mantenibilidad** | Cambios en UI no afectan la lógica de negocio. Los servicios del Back pueden evolucionar independientemente. |
| **Reutilización** | Un mismo Back puede alimentar múltiples Frontends (Blazor actual + WPF legacy + Mobile con MAUI). |
| **Clean Architecture** | Separa claramente responsabilidad: Datos → Dominio → Servicios → UI |

**Arquitectura del Frontend Blazor Server**

1. **Mantener Back intacto** - Servicios, repositorios, validadores no cambian
2. **Blazor Server** - UI interactiva mediante SignalR
3. **Async/Await en toda la cadena** - Desde la UI hasta la base de datos

```
Actualmente:
├── GestionAcademica.Back/       ← Sin cambios
├── GestionAcademica.Blazor/     ← Blazor Server (activo)
├── GestionAcademica.Front/      ← WPF (legacy)
└── GestionAcademica.Test/       ← Sin cambios
```

**Gestión de Dependencias en Blazor Server**

El Frontend Blazor usa los servicios del Backend mediante inyección de dependencias de .NET:

- **Back**: Define `GestionAcademica.Infrastructure.DependenciesProvider` (servicios core)
- **Blazor**: Configura servicios en `Program.cs` con `builder.Services.Add...`
- **Components**: Reciben servicios mediante `@inject` en Razor o constructor en code-behind

---

### 2.1.2. Diagrama de Capas del Sistema Completo
...
graph TB
    subgraph PRESENTATION["🎨 CAPA DE PRESENTACIÓN (Blazor Server)"]
        direction TB
        PAGES[Pages<br/>Estudiantes/Lista<br/>Docentes/Lista<br/>Inicio/Dashboard]
        COMPONENTS[Components<br/>PersonaCard<br/>PersonaForm<br/>Graficos]
        FORMDATA[FormData<br/>EstudianteFormData<br/>DocenteFormData]
        CSS[CSS/Estilos<br/>app.css<br/>Bootstrap]
    end
...
### 2.1.3. Diagrama de Componentes (Frontend + Backend)
...
%%{init: {'theme': 'dark', 'themeVariables': {'darkMode': true, 'background': '#1e1e1e', 'primaryColor': '#E3F2FD', 'secondaryColor': '#F3E5F5', 'tertiaryColor': '#E8F5E9', 'quaternaryColor': '#FFF3E0'}}}%%
graph TB
    subgraph FRONTEND["🖥️ FRONTEND BLAZOR"]
        ML[MainLayout]
        NM[NavMenu]
        
        subgraph ESTUDIANTES_MODULE["Módulo Estudiantes"]
            EL[Pages/Estudiantes/Lista]
            EEB[Pages/Estudiantes/Editar]
            EFD[EstudianteFormData]
        end
        
        subgraph DOCENTES_MODULE["Módulo Docentes"]
            DL[Pages/Docentes/Lista]
            DEB[Pages/Docentes/Editar]
            DFD[DocenteFormData]
        end
...
#### 📊 Flujo de Datos Completo (Ejemplo: Crear Estudiante Async)

```
1. Usuario click "Nuevo Estudiante" en Lista.razor
   ↓
2. Code-behind ejecuta Nav.NavigateTo("/estudiantes/editar/0")
   ↓
3. Editar.razor carga EstudianteFormData (parámetro Id=0 = nuevo)
   ↓
4. Usuario completa formulario (validación FluentValidation en tiempo real)
   ↓
5. Click "Guardar" → async Task GuardarAsync()
   ↓
6. formData.ToModel(Id) → Estudiante entity
   ↓
7. await Service.CrearAsync(estudiante)
   ↓
8. await ValidadorEstudiante.ValidarAsync(estudiante) → Result<Persona, DomainError>
   ↓
9. await Repository.CreateAsync(estudiante) → Persona guardada
   ↓
10. Cache.Add(id, estudiante)
    ↓
11. Result.Success → Nav.NavigateTo("/estudiantes")
    ↓
12. Lista.razor recarga: await CargarEstudiantesAsync()
    ↓
13. UI actualizada automáticamente (Blazor RenderTree)
```

**💡 Para estudiantes:**

Este flujo demuestra **separación perfecta de responsabilidades**:
- **Page (Blazor)**: Solo muestra datos y captura eventos
- **Code-behind**: Orquesta la lógica de UI (async/await)
- **FormData**: Valida datos con FluentValidation
- **Service**: Orquesta la lógica de negocio (backend, async)
- **Validator**: Valida reglas de negocio (backend)
- **Repository**: Persiste datos (async I/O)
...
```csharp
// ✅ BIEN: Inversión de dependencias (async/await)
public class PersonasService(
    IPersonasRepository repository,  // ← Inyectado (async)
    ICache<int, Persona> cache,      // ← Inyectado (async)
    IValidador<Persona> validador    // ← Inyectado (async)
)
{
    // Ahora puedo cambiar implementaciones sin tocar esta clase
    // Ahora puedo testear con mocks/fakes
    // Todos los métodos son async: await repository.GetAllAsync()
}
```
...
```csharp
public async Task<Result<Persona, DomainError>> CrearAsync(Persona persona, CancellationToken ct = default)
{
    return await ValidarPersonaAsync(persona, ct)           // ¿Es válida?
        .EnsureAsync(p => !await ExisteDniAsync(p.Dni),    // ¿DNI único?
                     PersonaErrors.AlreadyExists(persona.Dni))
        .EnsureAsync(p => !await ExisteEmailActivoAsync(p.Email), // ¿Email único?
                     PersonaErrors.AlreadyExists(persona.Email))
        .MapAsync(p => await repository.CreateAsync(p!, ct));     // Guardar
}
```

**💡 Para estudiantes - Desglose paso a paso (async):**

```csharp
// PASO 1: Validar (async)
Task<Result<Persona, DomainError>> resultado = ValidarPersonaAsync(persona, ct);
// Si falla → return Failure(ValidationError)
// Si OK → continúa con Persona válida

// PASO 2: EnsureAsync - Verificar condición (async)
resultado = resultado.EnsureAsync(
    async p => !await ExisteDniAsync(p.Dni),           // Condición: DNI no debe existir
    PersonaErrors.AlreadyExists(persona.Dni)            // Error si falla
);
// Si falla → return Failure(AlreadyExists)
// Si OK → continúa

// PASO 3: MapAsync - Transformar el valor (async I/O)
resultado = resultado.MapAsync(async p => await repository.CreateAsync(p!, ct));
// Solo se ejecuta si todo lo anterior fue Success
// Transforma Persona validada → Persona guardada (async I/O)
...
```csharp
// ✅ BIEN: GetAllAsync siempre tiene éxito
public async Task<IEnumerable<Persona>> GetAllAsync()
{
    return await repository.GetAllAsync(); // Devuelve [] si está vacío
}

// ❌ MAL: GetAllAsync con Result innecesario
public async Task<Result<IEnumerable<Persona>, DomainError>> GetAllAsync()
{
    var result = await repository.GetAllAsync();
    return Result.Success<IEnumerable<Persona>, DomainError>(result);
    // Complejidad innecesaria: nunca va a fallar
}

// ✅ BIEN: GetByIdAsync puede fallar
...
## 2.5 Async/Await en .NET

Este proyecto hace uso intensivo de programación asíncrona con async/await. A diferencia del enfoque síncrono tradicional, async/await permite que un hilo no se bloquee mientras espera una operación de I/O (lectura de archivos, consultas a bases de datos, peticiones HTTP, etc.).

### ¿Cómo funciona?

1. **async**: Modificador que marca un método como asíncrono. Permite usar `await` en su interior.
2. **await**: Suspende la ejecución del método hasta que la operación await completada finalice, mientras libera el hilo para atender otras peticiones.
3. **Task/Task\<T\>**: Representa una operación en curso. `Task` es void asíncrono, `Task\<T\>` devuelve un valor.
...
%%{init: {'theme': 'dark', 'themeVariables': {'darkMode': true, 'background': '#1e1e1e'}}}%%
sequenceDiagram
    actor C as Cliente (Blazor)
    participant P as Page/Component
    participant S as Service
    participant R as Repository
    participant DB as Base de Datos

    C->>P: 1. Solicita datos
    activate P
    P->>S: 2. await ObtenerAsync()
    activate S
    S->>R: 3. await GetAllAsync()
    activate R
    R->>DB: 4. SELECT query
    activate DB
    DB-->>R: 5. Resultados
    ...
    S-->>P: 7. Result\<List\<T\>\>
    deactivate S
    Note over P: 8. Hilo liberado mientras<br/>se espera la BD
    P-->>C: 9. Renderiza UI
    deactivate P
```
...
- **Escalabilidad**: El servidor Blazor puede atender más clientes simultáneos porque los hilos no se bloquean en I/O.
- **Responsividad**: La UI no se congela durante operaciones largas (importar/exportar CSV, backups).
- **Consistencia**: Toda la cadena de llamadas es async, desde la UI hasta la base de datos.
- **Result\<T\>**: Combinamos async/await con el patrón Result para propagar errores sin excepciones.

### Reglas que seguimos

1. **Async all the way**: Nunca usamos `.Result` o `.Wait()` — eso causa deadlocks.
2. **Task\<Result\<T\>\>**: Los servicios devuelven `Task\<Result\<T, DomainError\>\>`.
3. **Métodos CPU-bound**: Usamos `Task.Run` solo para trabajo intensivo de CPU (generación de informes HTML).
4. **Cancelación**: Los métodos aceptan `CancellationToken` para poder cancelar operaciones largas.
...
{
    // CRUD Básico heredado de ICrudRepository<TKey, TEntity> (todos async)
    // Task<Persona?> GetByIdAsync(int id);
    // Task<IEnumerable<Persona>> GetAllAsync();
    // Task<Persona?> CreateAsync(Persona entity);
    // Task<Persona?> UpdateAsync(int id, Persona entity);
    // Task<Persona?> DeleteAsync(int id);
    
    // Métodos específicos del dominio (todos async)
    Task<Persona?> GetByDniAsync(string dni);
    Task<bool> ExisteDniAsync(string dni);
    Task<bool> DeleteAllAsync();
    
    // Paginación y filtrado (async)
    Task<IEnumerable<Persona>> GetAllAsync(int page = 1, int size = int.MaxValue, EstadoRegistro estado = EstadoRegistro.Activos, CancellationToken ct = default);
}
```
...
public class PersonasMemoryRepository : IPersonasRepository
{
    ...
    public Task<Persona?> GetByIdAsync(int id)
    {
        ...
        return Task.FromResult(_diccionario.TryGetValue(id, out var persona) ? persona : null);
    }

    public Task<Persona?> GetByDniAsync(string dni)
    {
        _logger.Debug("Buscando persona con DNI: {Dni}", dni);
        return Task.FromResult(_diccionario.Values.FirstOrDefault(p => p.Dni == dni));
    }
    ...
    public Task<IEnumerable<Persona>> GetAllAsync(int page = 1, int size = int.MaxValue, 
                                       EstadoRegistro estado = EstadoRegistro.Activos, CancellationToken ct = default)
    {
...
### 3.3.3.1. Caché Dual (ID + DNI)

El sistema implementa **dos cachés LRU independientes** para optimizar diferentes tipos de búsquedas:

| Caché | Clave | Uso |
| ----- | ----- | ----- |
| `ICache<int, Persona>` | `int` (ID) | Búsquedas por `GetById(id)` |
| `ICache<string, Persona>` | `string` (DNI) | Búsquedas por `GetByDni(dni)` |
...
**Implementación en PersonasService:**

```csharp
public class PersonasService(
    ICache<int, Persona> cacheById,      // Para GetById
    ICache<string, Persona> cacheByDni   // Para GetByDni
) : IPersonasService {
    ...
    public Result<Persona, DomainError> GetById(int id) {
        ...
        // Si no está, buscar en repo y añadir a ambas caches
        if (repository.GetById(id) is { } persona) {
            cacheById.Add(id, persona);
            cacheByDni.Add(persona.Dni, persona);
            return Result.Success(persona);
        }
        return Result.Failure(PersonaErrors.NotFound(id.ToString()));
    }
    ...
    public Result<Persona, DomainError> GetByDni(string dni) {
        ...
        if (repository.GetByDni(dni) is { } persona) {
            ...
            cacheById.Add(persona.Id, persona);
            return Result.Success(persona);
        }
        return Result.Failure(PersonaErrors.NotFound(dni));
    }
}
```

**Invalidación de ambas caches:**

```csharp
// En Update: invalidar ambas caches
cacheById.Remove(id);
cacheByDni.Remove(persona.Dni);

// En Delete: invalidar ambas caches  
cacheById.Remove(id);
cacheByDni.Remove(persona.Dni);

// En Save: añadir a ambas caches
cacheById.Add(creada.Id, creada);
cacheByDni.Add(creada.Dni, creada);
```

**Diagrama de Arquitectura de la Caché Dual:**

```mermaid
flowchart TD
    subgraph GetByIdFlow["GetById(id)"]
        A1["GetById(id)"] --> B1{cacheById.Get}
        B1 -->|HIT| E1["Return cached"]
        B1 -->|MISS| F1["repository.GetById"]
        F1 --> G1["Add to cacheById + cacheByDni"]
    end
    
    subgraph GetByDniFlow["GetByDni(dni)"]
        A2["GetByDni(dni)"] --> B2{cacheByDni.Get}
        B2 -->|HIT| E2["Return cached"]
        B2 -->|MISS| F2["repository.GetByDni"]
        F2 --> G2["Add to cacheByDni + cacheById"]
    end
    
    subgraph Caches["Caches LRU"]
        C1["ICache int->Persona"]
        C2["ICache string->Persona"]
    end
...
1. **GetById(id)**: cacheById → si MISS, buscar en repo → añadir a ambas caches
2. **GetByDni(dni)**: cacheByDni → si MISS, buscar en repo → añadir a ambas caches
3. **Update/Delete**: invalidar ambas caches
4. **Save**: añadir a ambas caches
...
{
    // CRUD (todos async con Result<T, DomainError>)
    Task<Result<Persona, DomainError>> CrearAsync(Persona persona, CancellationToken ct = default);
    Task<Result<Persona, DomainError>> ActualizarAsync(int id, Persona persona, CancellationToken ct = default);
    Task<Result<Persona, DomainError>> EliminarAsync(int id, CancellationToken ct = default);
    Task<Result<Persona, DomainError>> ObtenerPorIdAsync(int id, CancellationToken ct = default);
    Task<Result<Persona, DomainError>> ObtenerPorDniAsync(string dni, CancellationToken ct = default);
    
    // Consultas (async)
    Task<Result<IEnumerable<Persona>, DomainError>> ObtenerTodosAsync(CancellationToken ct = default);
    Task<Result<IEnumerable<Persona>, DomainError>> ObtenerTodosOrderByAsync(
        TipoOrdenamiento orden = TipoOrdenamiento.Dni,
        Predicate<Persona>? filtro = null, CancellationToken ct = default);
    
    // Informes (async)
    Task<Result<InformeEstudiante, DomainError>> GenerarInformeEstudianteAsync(
        Ciclo? ciclo = null, Curso? curso = null, CancellationToken ct = default);
    Task<Result<InformeDocente, DomainError>> GenerarInformeDocenteAsync(
        Ciclo? ciclo = null, CancellationToken ct = default);
    
    // Import/Export (async)
    Task<Result<int, DomainError>> ImportarDatosAsync(CancellationToken ct = default);
    Task<Result<int, DomainError>> ExportarDatosAsync(CancellationToken ct = default);
    
    // Backup (async)
    Task<Result<string, DomainError>> RealizarBackupAsync(CancellationToken ct = default);
    Task<Result<int, DomainError>> RestaurarBackupAsync(string rutaZip, CancellationToken ct = default);
    Task<IEnumerable<string>> ListarBackupsAsync(CancellationToken ct = default);
...
El frontend del sistema está construido con **Blazor Server** utilizando el modelo de componentes interactivos de **.NET 10**. La comunicación entre el servidor y el cliente se realiza mediante **SignalR**, proporcionando una experiencia de usuario fluida con toda la lógica de negocio ejecutándose en el servidor.

---

## 6.1. Arquitectura Blazor Server

Blazor Server ejecuta la lógica de la aplicación en el servidor. Los eventos de UI (clics, cambios de formulario) se envían al servidor a través de una conexión **SignalR** persistente, donde el componente procesa la lógica y envía las actualizaciones del **RenderTree** al cliente.

### 6.1.1. Flujo de la UI en Blazor Server

```mermaid
%%{init: {'theme': 'dark', 'themeVariables': {'darkMode': true, 'background': '#1e1e1e'}}}%%
sequenceDiagram
    actor U as Usuario (Navegador)
    participant S as SignalR Connection
    participant C as Blazor Component
    participant Svc as Backend Service
    participant DB as Base de Datos
    
    U->>S: Click / Escribe
    S->>C: Envía evento UI
    C->>C: Procesa handler (C#)
    C->>Svc: await OperaciónAsync()
    Svc->>DB: Query/Command
    DB-->>Svc: Resultado
    Svc-->>C: Result\<T\>
    C->>C: Renderiza nuevo RenderTree
    C->>S: Diferencia HTML (diff)
    S->>U: Actualiza DOM (minimal)
    
    Note over C,Svc: Toda la lógica en servidor<br/>Cliente solo renderiza HTML
```

**Características clave:**

| Componente   | Responsabilidad                        | Ejemplo Blazor              |
| ------------ | -------------------------------------- | --------------------------- |
| **Page**     | Define ruta y layout                   | `@page "/estudiantes"`      |
| **Component**| Lógica + Renderizado                   | `EstudiantesLista.razor`    |
| **Code-behind** | Lógica en C# (partial class)        | `EstudiantesLista.razor.cs` |
| **Service**  | Lógica de negocio (inyectado via DI)   | `@inject IPersonasService`  |

### 6.1.2. DI en Blazor

Blazor usa el sistema de DI de **Microsoft.Extensions.DependencyInjection**. Los servicios se registran en `Program.cs`:
...
| Servicio     | Ciclo de Vida  | Uso en Blazor                    |
| ------------ | -------------- | -------------------------------- |
| **Singleton**| Toda la app    | Servicios sin estado             |
| **Scoped**   | Por circuito   | DialogService, state por usuario |
| **Transient**| Por resolución | Componentes efímeros             |
...
### 6.1.3. Diagrama de Arquitectura del Frontend Blazor

```mermaid
%%{init: {'theme': 'dark', 'themeVariables': {'darkMode': true, 'background': '#1e1e1e'}}}%%
graph TB
    subgraph UI["🖥️ BLAZOR COMPONENTS"]
        App[App.razor<br/>Router]
        ML[MainLayout.razor]
        IN[Pages/Inicio.razor]
        EL[Pages/Estudiantes/Lista.razor]
        EE[Pages/Estudiantes/Editar.razor]
        DL[Pages/Docentes/Lista.razor]
        DE[Pages/Docentes/Editar.razor]
    end
    
    subgraph CB["🧠 CODE-BEHIND"]
        ICode[Inicio.razor.cs]
        ECode[Editar.razor.cs]
        LCode[Lista.razor.cs]
    end
    ...
    style CB fill:#F57C00,stroke:#FF9800,color:#fff
    ...
    style ML fill:#1E3A5F,stroke:#4A90E2,color:#fff
    style IN fill:#1E3A5F,stroke:#4A90E2,color:#fff
    style EL fill:#1E3A5F,stroke:#4A90E2,color:#fff
    style EE fill:#1E3A5F,stroke:#4A90E2,color:#fff
    style DL fill:#1E3A5F,stroke:#4A90E2,color:#fff
    style DE fill:#1E3A5F,stroke:#4A90E2,color:#fff
    style ICode fill:#F57C00,stroke:#FF9800,color:#fff
    style ECode fill:#F57C00,stroke:#FF9800,color:#fff
    style LCode fill:#F57C00,stroke:#FF9800,color:#fff
...
**Flujo de dependencias (DI en Blazor):**

```mermaid
%%{init: {'theme': 'dark', 'themeVariables': {'darkMode': true, 'background': '#1e1e1e'}}}%%
graph LR
    A[Program.cs] -->|builder.Services| B[ServiceProvider]
    B -->|@inject| C[Code-behind<br/>ComponentBase]
    C -->|@inject| D[Services]
    D -->|@inject| E[Repositories]
    
    C -->|@bind / @event| F[.razor Component]
...
// Componentes se resuelven automáticamente via @inject
...
Los componentes Blazor se definen en archivos `.razor` y combinan HTML y C# en un mismo archivo, con la lógica en un archivo **code-behind** `.razor.cs`. Cada componente es autónomo y puede recibir parámetros, manejar eventos y renderizar UI.

### Páginas vs Componentes

| Concepto       | Descripción                                      | Archivo                          |
| -------------- | ------------------------------------------------ | -------------------------------- |
| **Página**     | Componente con ruta (`@page`), entrada a sección | `Estudiantes/Lista.razor`        |
| **Componente** | Bloque reutilizable, sin ruta                    | `Shared/PersonaCard.razor`       |
| **Layout**     | Plantilla común (header, nav, footer)            | `Shared/MainLayout.razor`        |
| **Code-behind**| Lógica del componente en C# partial class        | `Lista.razor.cs`                 |

### Directivas Clave

| Directiva             | Propósito                                    |
| --------------------- | -------------------------------------------- |
| `@page "/ruta"`       | Define la ruta de navegación de la página    |
| `@rendermode InteractiveServer` | Renderizado interactivo en servidor |
| `@inject IServicio`   | Inyecta un servicio mediante DI              |
| `@bind-Propiedad`     | Binding bidireccional entre UI y propiedad   |
| `@onclick`            | Manejo de eventos de clic                    |
| `@code { }`           | Bloque de código C# en el mismo archivo      |

### Componentes Principales

| Componente                 | Ruta                | Responsabilidad                        |
| -------------------------- | ------------------- | -------------------------------------- |
| `Pages/Inicio.razor`       | `/`                 | Dashboard con estadísticas             |
| `Pages/Estudiantes/Lista.razor` | `/estudiantes` | Listado y búsqueda de estudiantes      |
| `Pages/Estudiantes/Editar.razor` | `/estudiantes/editar/{id}` | Formulario de creación/edición |
| `Pages/Docentes/Lista.razor` | `/docentes`       | Listado y búsqueda de docentes         |
| `Pages/Docentes/Editar.razor` | `/docentes/editar/{id}` | Formulario de creación/edición   |
| `Pages/ImportExport.razor` | `/import-export`    | Importación y exportación de datos     |
| `Pages/Graficos.razor`    | `/graficos`         | Visualización de gráficos estadísticos |
| `Pages/Gestion.razor`     | `/gestion`          | Backup, restauración, informes         |

### Ejemplo: Componente Lista de Estudiantes

```razor
@page "/estudiantes"
@rendermode InteractiveServer
@inject IPersonasService Service
@inject IDialogService Dialog

<h3>👨‍🎓 Estudiantes</h3>

<button @onclick="NuevoAsync">➕ Nuevo</button>
    ...
    <tbody>
        @foreach (var est in estudiantes)
        {
            <tr>
                ...
                <td>@est.Email</td>
                <td>@est.NotaMedia.ToString("N2")</td>
                <td>
                    <button @onclick="() => EditarAsync(est.Id)">✏️</button>
                    <button @onclick="() => EliminarAsync(est.Id)">🗑️</button>
                </td>
            </tr>
        }
    </tbody>
...
@code {
    ...
    private async Task CargarEstudiantesAsync()
    {
        ...
        result.Match(
            onSuccess: data => estudiantes = data.OfType<Estudiante>().ToList(),
            onFailure: err => Dialog.ShowError(err.Message)
        );
    }

    private async Task NuevoAsync()
    {
        Navigation.NavigateTo("/estudiantes/editar/0");
    }
    ...
    {
        if (!await Dialog.ShowConfirmAsync("¿Eliminar estudiante?")) return;
        var result = await Service.EliminarAsync(id);
        result.Match(
            onSuccess: _ => await CargarEstudiantesAsync(),
            onFailure: err => Dialog.ShowError(err.Message)
        );
    }
}
...
Blazor usa la directiva `@bind` para binding bidireccional:

```razor
<input @bind="searchText" @bind:event="oninput" placeholder="🔍 Buscar..." />
...
@code {
    ...
    {
        var result = await Service.BuscarAsync(searchText);
        // Actualizar lista con resultado
    }
}
```
...
| Directiva Blazor      | Comportamiento                          | Equivalente WPF (antiguo)           |
| -------------------- | --------------------------------------- | ----------------------------------- |
| `@valor`            | OneWay: muestra el valor                | `{Binding Valor, Mode=OneWay}`      |
| `@bind-valor`       | TwoWay: sincroniza UI ↔ C#              | `{Binding Valor, Mode=TwoWay}`      |
| `@bind-valor:event="oninput"` | TwoWay en cada tecla        | `UpdateSourceTrigger=PropertyChanged` |
| `@onclick`          | Evento de clic                          | `Command="{Binding ...}"`           |

---

### 6.2.7. Diagrama de Navegación entre Páginas

```mermaid
%%{init: {'theme': 'dark', 'themeVariables': {'darkMode': true, 'background': '#1e1e1e'}}}%%
graph TD
    Start([App.razor]) --> Main[MainLayout.razor<br/>NavMenu]
    
    Main -->|/| Dash[Inicio.razor<br/>Dashboard]
    Main -->|/estudiantes| Est[Estudiantes/Lista.razor]
    Main -->|/docentes| Doc[Docentes/Lista.razor]
    
    Est -->|Nuevo| NewE[Estudiantes/Editar.razor<br/>id=0]
    Est -->|Editar| EditE[Estudiantes/Editar.razor<br/>id={id}]
    
    Doc -->|Nuevo| NewD[Docentes/Editar.razor<br/>id=0]
    Doc -->|Editar| EditD[Docentes/Editar.razor<br/>id={id}]
    
    NewE -->|Guardar OK| Est
    EditE -->|Guardar OK| Est
    NewD -->|Guardar OK| Doc
    EditD -->|Guardar OK| Doc
    
    Est -.->|Servicio DI scoped| Dash
    Doc -.->|Servicio DI scoped| Dash
...
1. **App.razor** renderiza `MainLayout.razor` con `NavMenu`
2. **NavMenu** ofrece enlaces a las páginas principales (`/`, `/estudiantes`, `/docentes`)
3. Usuario navega a una ruta → Blazor activa el componente asociado
4. Desde listas, se navega a las páginas de edición con `NavigationManager.NavigateTo()`
5. Al guardar, se redirige de vuelta a la lista
6. El Dashboard se actualiza mediante un servicio DI scoped, sin mensajería

---

## 8. Lógica de Componentes (Code-Behind)

La lógica de los componentes Blazor se implementa en archivos **code-behind** (`.razor.cs`) como clases **partial** que heredan de `ComponentBase`. Esto separa la lógica C# del marcado HTML/Razor.
...
%%{init: {'theme': 'dark', 'themeVariables': {'darkMode': true, 'background': '#1e1e1e'}}}%%
graph TB
    A[SetParametersAsync] --> B[OnInitializedAsync]
    B --> C[OnParametersSetAsync]
    C --> D[Render] 
    D --> E[OnAfterRenderAsync firstTime]
    E --> F[StateHasChanged]
    F --> G[Render again]
    G -.->|User Event| H[Event Handler]
    H --> F
```

### Métodos de ciclo de vida principales

| Método                   | Cuándo se ejecuta                                  | Uso típico                    |
| ------------------------ | -------------------------------------------------- | ----------------------------- |
| `OnInitializedAsync`     | Una vez, cuando el componente se inicializa        | Cargar datos iniciales        |
| `OnParametersSetAsync`   | Cuando se establecen/actualizan los parámetros      | Reaccionar a cambios de ruta  |
| `OnAfterRenderAsync`     | Después de cada renderizado                        | JS interop, inicializar charts |
| `Dispose` / `DisposeAsync`| Cuando el componente se destruye (implementar `IAsyncDisposable`) | Limpiar recursos |

### Ejemplo: ListaEstudiantes.razor.cs

```csharp
public partial class ListaEstudiantes : ComponentBase, IAsyncDisposable
{
    [Inject] private IPersonasService Service { get; set; } = null!;
    ...
    private async Task EliminarAsync(int id)
    {
        if (!await Dialog.ShowConfirmAsync("¿Eliminar?")) return;
...
### Validación con FluentValidation en Blazor

Blazor se integra con FluentValidation para validación en tiempo real:

```csharp
public class EstudianteFormData
{
    [Required(ErrorMessage = "El DNI es obligatorio")]
    [RegularExpression(@"^\d{8}[A-Z]$", ErrorMessage = "Formato DNI inválido")]
...
public class EstudianteValidator : AbstractValidator<EstudianteFormData>
{
    public EstudianteValidator()
    {
        RuleFor(x => x.Dni).NotEmpty().Matches(@"^\d{8}[A-Z]$");
...
### Mostrar errores de Result\<T\> en la UI
...
@code {
    ...
    {
        var result = await Service.CrearAsync(estudiante);
        result.Match(
            onSuccess: _ => errorMessage = null,
...
**Con properties simples (estilo Blazor):**
...
public string Nombre
{
    ...
    {
        _nombre = value;
        // En Blazor, StateHasChanged() se llama automáticamente
        // tras eventos de UI (@onclick, @bind, etc.)
    }
}
```

**Con lógica adicional en setter:**
...
public string SearchText
{
    ...
    {
        _searchText = value;
        // Blazor re-renderiza automáticamente al cambiar el valor
        // No necesitas INotifyPropertyChanged
    }
}
```

En Blazor no necesitamos `INotifyPropertyChanged` ni `[ObservableProperty]` porque el framework detecta automáticamente los cambios tras eventos de UI (`@onclick`, `@bind`) y llama a `StateHasChanged()` implícitamente.

---

### 6.4.2. Métodos Asíncronos con Eventos
...
**Evento con condición de habilitación:**

```razor
<button @onclick="EditarAsync" disabled="@(selectedEstudiante is null)">
    ✏️ Editar
</button>
```
...
{
    if (selectedEstudiante is null) return;
    await Task.CompletedTask;
}
```

**Evento asíncrono con servicio:**

```csharp
private async Task GuardarAsync()
{
    var result = await _personasService.CreateAsync(estudiante);
    result.Match(
        onSuccess: _ => { /* éxito */ },
        onFailure: err => { /* error */ }
    );
...
**Binding con condición:**
```razor
<button @onclick="() => EditarAsync(est.Id)" 
        disabled="@(est.IsDeleted)">
    ✏️
</button>
```
...
### 6.4.3. Comunicación entre Componentes

**Problema:** Necesitamos que cuando se crea/edita/elimina un estudiante, el Dashboard se actualice automáticamente sin tener una referencia directa.

**Solución:** Usar servicios DI con scope por circuito (Scoped) o eventos entre componentes.

**Comunicación padre → hijo con parámetros:**

```razor
<!-- Padre -->
<ChildComponent @bind-Value="propiedad" />
```

**Comunicación hijo → padre con EventCallback:**

```razor
<!-- Hijo -->
<button @onclick="OnChange.InvokeAsync()">Notificar</button>
...
**Comunicación entre componentes no relacionados (DI Service):**

```csharp
// Servicio compartido (scoped por circuito)
public class PersonaChangedService
{
    public event Action? OnChanged;
    public void NotifyChanged() => OnChanged?.Invoke();
}

// Componente A notifica
@inject PersonaChangedService PersonaChanged
@code {
    private async Task GuardarAsync()
    {
        await Service.CreateAsync(student);
        PersonaChanged.NotifyChanged();
    }
}
...
@code {
    ...
    {
        PersonaChanged.OnChanged += OnPersonaChanged;
    }
    private async void OnPersonaChanged()
    {
...
**Ventajas de usar DI + EventCallback:**

| Característica      | Beneficio                                          |
| ------------------- | -------------------------------------------------- |
| **Desacoplamiento** | Componentes no se conocen entre sí                  |
| **Type-safe**       | Parámetros tipados                                 |
| **Sin memory leaks**| Dispose automático con `DisposeAsync`              |
| **Scope por circuito**| Estado aislado por usuario                       |
| **Testeable**       | Fácil mockear para testing                         |

---

# 9. Arquitectura Blazor (Server)

Blazor Server ejecuta la lógica de la aplicación en el servidor y actualiza el DOM del cliente mediante una conexión **SignalR** persistente. Esto proporciona una experiencia SPA sin necesidad de descargar WebAssembly ni escribir JavaScript.

## 9.1. Circuito Blazor Server

Cada cliente conectado tiene un **circuito** que mantiene el estado del componente en el servidor:
...
sequenceDiagram
    ...
    participant S as Servidor Blazor
    participant BC as Circuito
    
    B->>SR: 1. Conecta /_blazor
    SR->>S: 2. Abre circuito
    S->>BC: 3. Crea componente raíz
    BC->>BC: 4. RenderTree inicial
    BC-->>SR: 5. Diferencia HTML
    SR-->>B: 6. Actualiza DOM
    
    Note over B,BC: Circuito establecido
    
    B->>SR: 7. Clic botón
    SR->>BC: 8. Evento UI
    BC->>BC: 9. Ejecuta handler C#
    BC->>BC: 10. StateHasChanged
    BC-->>SR: 11. Nuevo diff HTML
    SR-->>B: 12. Actualiza DOM
    
    Note over B,BC: El circuito persiste hasta<br/>que el usuario cierra pestaña<br/>o timeout supera los 30s
```

## 9.2. Gestión de Estado

En Blazor Server, el estado vive en el servidor dentro del circuito:

| Tipo de Estado   | Dónde se almacena                         | Persistencia               |
| ---------------- | ----------------------------------------- | --------------------------- |
| **Componente**   | Propiedades del componente (servidor)     | Durante el circuito         |
| **Servicio DI**  | Scoped (por circuito) o Singleton         | Por circuito o toda la app  |
| **Navegación**   | URL en el navegador                       | Persistente                 |
| **Persistente**  | Base de datos (a través de servicios)     | Entre sesiones              |

## 9.3. Árbol de Componentes

Blazor construye un **árbol de componentes** (RenderTree) que se actualiza eficientemente:

```
App.razor
└── MainLayout.razor (NavMenu + contenido)
    ├── Inicio.razor (/)
    │   └── Estadisticas.razor
    ├── Estudiantes/Lista.razor (/estudiantes)
    │   └── PersonaCard.razor (para cada estudiante)
    ├── Estudiantes/Editar.razor (/estudiantes/editar/{id})
    │   └── PersonaForm.razor
    ├── Docentes/Lista.razor (/docentes)
    └── Graficos.razor (/graficos)
```

## 9.4. Navegación con NavigationManager

```razor
@inject NavigationManager Nav

<button @onclick="IrAEstudiantes">Estudiantes</button>

@code {
    private void IrAEstudiantes()
    {
        Nav.NavigateTo("/estudiantes");
    }

    private void IrAEditar(int id)
    {
        Nav.NavigateTo($"/estudiantes/editar/{id}");
    }
}
```

La navegación **no recarga la página** — Blazor intercepta los clics y actualiza el componente raíz manteniendo el circuito SignalR activo.

## 9.5. Estado y Renderizado

- **StateHasChanged()**: Notifica a Blazor que el componente debe re-renderizarse. Se llama automáticamente tras eventos de UI.
- **RenderTree diff**: Blazor calcula la diferencia entre el árbol anterior y el nuevo, enviando solo los cambios mínimos al DOM del cliente.
- **Circuit reconnection**: Si SignalR se desconecta, Blazor muestra un mensaje de reconexión. Al reconectarse, restaura el circuito automáticamente.
...
### 8.1.1. EstudianteFormData

Clase que representa los datos del formulario de estudiante con propiedades simples:
...
public class EstudianteFormData
{
    ...
    public List<NotaFormData> Notas { get; set; } = new();
...
```csharp
public class NotaFormData
{
    public int AsignaturaId { get; set; }
    public string NombreAsignatura { get; set; } = string.Empty;
    public double Calificacion { get; set; }
    public TipoEvaluacion TipoEvaluacion { get; set; } = TipoEvaluacion.Ordinaria;
}
```

---

### 8.1.2. DocenteFormData

Similar estructura pero adaptada a los campos de Docente:

```csharp
public class DocenteFormData
{
    public string Dni { get; set; } = string.Empty;
    public string Nombre { get; set; } = string.Empty;
    public string? Telefono { get; set; }
...
### 8.1.3. Validación con FluentValidation

FluentValidation es la librería estándar para validación en Blazor. Los validadores se definen como clases separadas que heredan de `AbstractValidator<T>`.
...
```csharp
builder.Services.AddValidatorsFromAssemblyContaining<EstudianteValidator>();
```

**Validador para EstudianteFormData:**
...
public class EstudianteValidator : AbstractValidator<EstudianteFormData>
{
    public EstudianteValidator()
    {
        RuleFor(x => x.Dni)
            ...
            .Must(ValidarDniCompleto).WithMessage("La letra del DNI no es correcta");
        ...
        RuleFor(x => x.Telefono)
            .Matches(@"^[0-9]{9}$").When(x => !string.IsNullOrEmpty(x.Telefono))
            .WithMessage("Debe tener 9 dígitos");

        RuleFor(x => x.FechaNacimiento)
            .NotNull().WithMessage("La fecha es obligatoria")
            .Must(f => f <= DateTime.Now).WithMessage("No puede ser futura");

        RuleFor(x => x.Ciclo).NotNull().WithMessage("El ciclo es obligatorio");
        RuleFor(x => x.Curso).NotNull().WithMessage("El curso es obligatorio");
    }
...
**Validador para DocenteFormData:**

```csharp
public class DocenteValidator : AbstractValidator<DocenteFormData>
{
    public DocenteValidator()
    {
        RuleFor(x => x.Dni).NotEmpty().Length(9);
        RuleFor(x => x.Nombre).NotEmpty().MinimumLength(2);
        RuleFor(x => x.TipoDocente).NotNull();
        RuleFor(x => x.Especialidad).NotEmpty();
        RuleFor(x => x.AniosExperiencia).InclusiveBetween(0, 50);
    }
}
```
...
| Ventaja                    | Descripción                                                 |
| -------------------------- | ----------------------------------------------------------- |
| **Separación**             | Validación fuera del modelo, en clases dedicadas            |
| **Composición**            | Reglas reutilizables y encadenables (`RuleSet`, `Include`)  |
| **Integración Blazor**     | `FluentValidationValidator` componente nativo               |
| **Mensajes personalizados**| `WithMessage()` para mensajes específicos                   |
| **Validación condicional** | `When()`, `Unless()` para reglas contextuales               |
| **Pruebas unitarias**      | Fácil testear validadores de forma aislada                  |

---

### 8.1.4. ¿Por qué FormData en vez de ViewModel?
...
**❌ SIN FormData (todo en el componente):**

```csharp
// Componente con 30+ propiedades, validación y lógica mezclada
public partial class EditarEstudiante : ComponentBase
{
...
```csharp
// FormData: Solo datos y conversión (100 líneas)
public class EstudianteFormData
{
...
// Validator: Validación separada (50 líneas)
public class EstudianteValidator : AbstractValidator<EstudianteFormData> { ... }

// Componente: Solo lógica de UI y orquestación (150 líneas)
public partial class EditarEstudiante : ComponentBase
{
    private EstudianteFormData formData = new();
    
    private async Task GuardarAsync()
    {
        var estudiante = formData.ToModel();
        var result = await Service.CreateAsync(estudiante);
    }
}
```
...
### 8.1.5. Diagrama de Clases: FormData y Validación FluentValidation
...
classDiagram
    ...
    class EstudianteValidator {
        +EstudianteValidator()
        -bool ValidarDniCompleto(string)
    }
    ...
    EstudianteValidator ..> EstudianteFormData : valida
    DocenteValidator ..> DocenteFormData : valida
```

---

## 8.2. Validación en Tiempo Real
...
### 8.2.1. EditForm + FluentValidation
...
<EditForm Model="formData" OnValidSubmit="GuardarAsync">
    ...
    <div>
        <label>DNI *</label>
        <InputText @bind-Value="formData.Dni" @bind-Value:event="oninput" />
        <ValidationMessage For="() => formData.Dni" />
    </div>
...
| Componente                 | Función                                           |
| -------------------------- | ------------------------------------------------- |
| **EditForm**               | Contenedor de formulario con validación           |
| **FluentValidationValidator**| Integra FluentValidation con Blazor             |
| **InputText**              | Campo de texto con `@bind-Value`                  |
| **@bind-Value:event="oninput"** | Actualiza en cada tecla (tiempo real)       |
| **ValidationMessage**      | Muestra el mensaje de error del validador         |

**¿Qué hace Blazor automáticamente?**

1. Usuario escribe en el InputText
2. Blazor actualiza `formData.Dni` (por `@bind-Value:event="oninput"`)
3. FluentValidation ejecuta `EstudianteValidator` en cada cambio
4. Si hay error → `ValidationMessage` muestra el mensaje con clase CSS `validation-message`
5. Si no hay error → sin mensaje, campo válido

**Estilos visuales (CSS):**

```css
.validation-message { color: #dc3545; font-size: 0.875em; }
...
### 8.2.2. @bind-Value:event="oninput"

**Opciones de actualización:**

| Evento        | Cuándo se actualiza                           | Uso típico                          |
| ------------- | --------------------------------------------- | ----------------------------------- |
| **oninput**   | En cada tecla                                 | Validación en tiempo real, búsqueda |
| **onchange**  | Al salir del campo (por defecto)              | Validación al terminar de escribir  |

**Ejemplo comparativo:**

```razor
<!-- ❌ VALIDACIÓN TARDÍA (al salir del campo) -->
<InputText @bind-Value="formData.Dni" />
...
sequenceDiagram
    actor U as Usuario
    participant IP as InputText
    participant B as Blazor Renderer
    participant FD as FormData
    participant FV as FluentValidation
    participant VM as ValidationMessage

    U->>IP: Escribe "1234567"
    IP->>B: oninput event
    B->>FD: formData.Dni = "1234567"
    B->>FV: Validate(formData)
    FV->>FV: EstudianteValidator.Validate()
    FV-->>B: Error "El DNI debe tener 9 caracteres"
    B->>VM: Mostrar mensaje de error
```
...
### 8.2.3. Mensajes de Error por Campo

**Customización de mensajes (validador FluentValidation):**

```csharp
RuleFor(x => x.Dni)
    .NotEmpty().WithMessage("El DNI es obligatorio")
    .Length(9).WithMessage("El DNI debe tener 9 caracteres (8 dígitos + letra)")
    .Must(dni => char.IsLetter(dni[8])).WithMessage("El último carácter debe ser una letra")
    .Must(ValidarDniCompleto).WithMessage("La letra del DNI no es correcta");
```
...
%%{init: {'theme': 'dark', 'themeVariables': {'darkMode': true, 'background': '#1e1e1e'}}}%%
graph TD
    A[RuleFor Dni] --> B{NotEmpty?}
    B -->|No| E1["❌ El DNI es obligatorio"]
    B -->|Sí| C{Length == 9?}
    C -->|No| E2["❌ El DNI debe tener 9 caracteres"]
    C -->|Sí| D{Letra correcta?}
...
### 8.2.4. Validación Global con EditForm
...
@code {
    ...
    private async Task GuardarAsync()
    {
        ...
        var result = await Service.CreateAsync(estudiante);
        result.Match(
            onSuccess: _ => Dialog.ShowSuccess("✅ Guardado correctamente"),
            onFailure: error => Dialog.ShowError(error.Message)
        );
    }
}
```

**EditForm maneja automáticamente:**
- `OnValidSubmit` solo se ejecuta si todas las reglas de validación pasan
- `FluentValidationValidator` valida con las reglas registradas en DI
- `ValidationMessage` muestra errores por campo
- Clases CSS `valid`/`invalid` se aplican automáticamente

---

### 8.2.5. Diagrama de Actividad: Validación Frontend Blazor

```mermaid
%%{init: {'theme': 'dark', 'themeVariables': {'darkMode': true, 'background': '#1e1e1e'}}}%%
flowchart TD
    Start([Usuario escribe en campo]) --> Trigger{@bind-Value:event}
    
    Trigger -->|oninput| Update[Actualizar formData.Property]
    
    Update --> Notify[Blazor Renderer detecta cambio]
    Notify --> Validate[FluentValidationValidator<br/>ejecuta validador]
    
    Validate --> Check{Resultado}
    
    Check -->|Invalid| ShowError[ValidationMessage<br/>muestra error]
    Check -->|Valid| ShowOK[Sin mensaje de error]
    
    ShowError --> FieldState[CSS class: invalid]
    ShowOK --> FieldState[CSS class: valid]
    
    FieldState --> EditForm[EditForm evalua OnValidSubmit]
    EditForm -->|Válido| Submit[Boton Submit habilitado]
    EditForm -->|Inválido| Block[Boton Submit deshabilitado]
    ...
    style Submit fill:#388E3C,stroke:#4CAF50,color:#fff
    style Block fill:#B71C1C,stroke:#F44336,color:#fff
    style EndFlow fill:#757575,stroke:#9E9E9E,color:#fff
    style Trigger fill:#1E3A5F,stroke:#4A90E2,color:#fff
    style Update fill:#1E3A5F,stroke:#4A90E2,color:#fff
    style Notify fill:#F57C00,stroke:#FF9800,color:#fff
    style Validate fill:#F57C00,stroke:#FF9800,color:#fff
    style Check fill:#F57C00,stroke:#FF9800,color:#fff
    style FieldState fill:#1E3A5F,stroke:#4A90E2,color:#fff
    style EditForm fill:#7B1FA2,stroke:#AB47BC,color:#fff
```
...
| -------- | ------------------------------------- | ----------------------------------- |
| **[1]**  | Usuario escribe                       | Evento `oninput` del InputText      |
| **[2]**  | `@bind-Value:event="oninput"`         | Actualiza propiedad en cada tecla   |
| **[3]**  | `formData.Property = value`           | Setter de propiedad                  |
| **[4]**  | Blazor Renderer                       | Detecta cambio y re-renderiza       |
| **[5]**  | FluentValidationValidator             | Ejecuta `EstudianteValidator`       |
| **[6]**  | `AbstractValidator.Validate()`        | Evalúa todas las reglas             |
| **[7]**  | `WithMessage("...")`                  | Retorna mensaje de error o vacío    |
| **[8]**  | ValidationMessage                     | Muestra error con CSS `invalid`     |
| **[9]**  | EditForm.OnValidSubmit                | Solo se ejecuta si todo es válido   |
...
↓
@bind-Value:event="oninput" → actualización inmediata
↓
formData.Dni = "1234"
↓
FluentValidation ejecuta EstudianteValidator
↓
RuleFor(x => x.Dni).Length(9) → FALLO
↓
ValidationMessage: "El DNI debe tener 9 caracteres"
↓
InputText muestra clase CSS .invalid (borde rojo)
↓
EditForm.OnValidSubmit no se ejecutará
↓
Botón Guardar no activa envío
```
...
Los servicios de UI proporcionan funcionalidades transversales a los componentes Blazor, manteniendo el código limpio y reutilizable.

---

## 10.1. Sistema de Navegación

La navegación en Blazor se basa en rutas (`@page`) y el servicio `NavigationManager` para cambiar entre páginas sin recargar el navegador.

### 10.1.1. Enrutamiento con @page

Cada página Blazor declara su ruta con la directiva `@page`:
...
| ------------------------------- | ------------------------ |
| `Pages/Inicio.razor`            | `/`                      |
| `Pages/Estudiantes/Lista.razor` | `/estudiantes`           |
| `Pages/Estudiantes/Editar.razor`| `/estudiantes/editar/{id}`|
| `Pages/Docentes/Lista.razor`    | `/docentes`              |

### 10.1.2. NavMenu con NavigationManager

```razor
<!-- MainLayout.razor o NavMenu.razor -->
<nav>
    <a href="/">🏠 Dashboard</a>
...
<button @onclick="() => Navigation.NavigateTo(\"/estudiantes/editar/0\")">
    ➕ Nuevo Estudiante
</button>
```

### 10.1.3. Parámetros de Ruta
...
@code {
    ...
    protected override async Task OnInitializedAsync()
    {
        if (Id > 0)
        {
            ...
            result.Match(
                onSuccess: data => formData = EstudianteFormData.FromModel(data),
                onFailure: err => Dialog.ShowError(err.Message)
            );
        }
    }
...
### 10.1.4. Paso de Parámetros entre Componentes

**Parámetros de componente (padre → hijo):**

```razor
<EstudianteCard Estudiante="est" 
                @rendermode="InteractiveServer" />

@code {
    private Estudiante est = new();
}
```

```csharp
// EstudianteCard.razor.cs
public partial class EstudianteCard : ComponentBase
{
    [Parameter] public Estudiante Estudiante { get; set; } = null!;
}
```

**EventCallback (hijo → padre):**

```razor
<ChildComponent OnEdit="() => EditarAsync(est.Id)" />
...
### 10.1.5. Flujo de navegación

```
Usuario → Clic enlace/botón → NavigationManager.NavigateTo() → 
Blazor Router → Carga página @page → OnInitializedAsync → Render
```

**Comparación de enfoques:**

| Enfoque              | Ventajas                                  | Uso en Blazor                        |
| -------------------- | ----------------------------------------- | ------------------------------------ |
| **@page + @onclick** | Type-safe, explícito, simple              | Navegación entre páginas             |
| **Parámetros**       | Flexible, se pasa por ruta                | Editar con ID específico             |
| **EventCallback**    | Comunicación hijo → padre                 | Acciones desde componentes hijos     |

---

### 10.1.7. Diálogos y Confirmaciones

En Blazor los diálogos se manejan mediante servicios de UI o componentes modales.
...
@code {
    ...
    {
        var estudiante = formData.ToModel();
        var result = await Service.UpdateAsync(estudiante.Id, estudiante);
        result.Match(
            onSuccess: _ =>
            {
                Dialog.ShowSuccess("✅ Guardado correctamente");
...
```razor
<button @onclick="EliminarAsync">🗑️ Eliminar</button>

@code {
    private async Task EliminarAsync()
    {
        if (!await Dialog.ShowConfirmAsync("¿Está seguro?")) return;

        var result = await Service.DeleteAsync(estudianteId);
        result.Match(
            onSuccess: _ => await CargarDatosAsync(),
            onFailure: err => Dialog.ShowError(err.Message)
        );
    }
}
...
La comunicación entre componentes Blazor se realiza mediante parámetros, EventCallback, servicios DI y CascadingParameter.

### 10.2.1. Parámetros y EventCallback (padre ↔ hijo)

**Padre → Hijo con [Parameter]:**

```razor
<EstudianteCard Estudiante="est" @rendermode="InteractiveServer" />
```
...
**Hijo → Padre con EventCallback:**

```razor
<ConfirmDialog OnConfirm="HandleConfirm" Message="¿Continuar?" />
```
...
### 10.2.2. Servicio Compartido (Scoped)

Para comunicación entre componentes no relacionados (mismo circuito):

```csharp
// Servicio con ámbito Scoped (uno por circuito)
public class PersonaChangedService
{
...
```csharp
builder.Services.AddScoped<PersonaChangedService>();
```
...
private async Task GuardarAsync()
{
    var result = await Service.CreateAsync(formData.ToModel());
    result.Match(
        onSuccess: _ => await PersonaChanged.NotifyChangedAsync(),
        onFailure: err => Dialog.ShowError(err.Message)
    );
}
```
...
protected override async Task OnInitializedAsync()
{
    PersonaChanged.OnChangedAsync += OnPersonaChangedAsync;
    await CargarDatosAsync();
}

private async Task OnPersonaChangedAsync()
{
    await CargarDatosAsync();
    StateHasChanged();
}
...
private async Task CargarDatosAsync()
{
    ...
    result.Match(
        onSuccess: data =>
        {
            estudiantes = data.OfType<Estudiante>().ToList();
...
### 10.2.3. CascadingParameter

Para pasar datos jerárquicamente sin prop drilling:

**Componente raíz:**

```razor
<CascadingValue Value="usuarioActual">
    <Router ... />
</CascadingValue>
...
```csharp
[CascadingParameter] protected Usuario UsuarioActual { get; set; } = null!;
```

**Ventajas de los patrones Blazor:**

| Patrón               | Uso                                    | Ámbito                  |
| -------------------- | -------------------------------------- | ----------------------- |
| **Parameter**        | Padre → Hijo, datos simples            | Por componente          |
| **EventCallback**    | Hijo → Padre, notificaciones           | Por componente          |
| **DI Service**       | Componentes no relacionados            | Scoped (por circuito)   |
| **CascadingParameter**| Datos jerárquicos sin prop drilling  | Subárbol de componentes |

```mermaid
%%{init: {'theme': 'dark', 'themeVariables': {'darkMode': true, 'background': '#1e1e1e'}}}%%
sequenceDiagram
    actor U as Usuario
    participant EV as EstudiantesPage
    participant S as PersonasService
    participant PS as PersonaChangedService
    participant DV as DashboardPage

    U->>EV: Click "Guardar"
    EV->>S: CreateAsync(estudiante)
    S-->>EV: Result.Success
    
    EV->>PS: NotifyChangedAsync()
    PS->>DV: OnChangedAsync event
    
    DV->>DV: OnPersonaChangedAsync()
    DV->>S: ObtenerTodosAsync()
    S-->>DV: List<Persona>
    DV->>DV: LoadStatistics()
    DV->>DV: StateHasChanged()
    
    Note over DV: Dashboard actualizado sin referencia directa al emisor
```

# 11. Binding y Eventos en Blazor

## 11.1. Data Binding en Blazor

Blazor usa binding bidireccional con la directiva `@bind`, que conecta automáticamente un elemento HTML con una propiedad C#.

### 11.1.1. One-Way vs Two-Way Binding

**Tipos de binding en Blazor:**

| Modo               | Dirección                  | Uso típico                           | Ejemplo Blazor                  |
| ------------------ | -------------------------- | ------------------------------------ | ------------------------------- |
| **One-Way**        | C# → HTML                  | Mostrar datos de solo lectura        | `@estudiante.Nombre`            |
| **Two-Way**        | C# ↔ HTML                  | Edición de datos                     | `@bind-Value="formData.Nombre"` |
| **Event-binding**  | HTML → C#                  | Manejo de eventos                    | `@onclick="Handler"`            |
...
%%{init: {'theme': 'dark', 'themeVariables': {'darkMode': true, 'background': '#1e1e1e'}}}%%
graph LR
    CS[Propiedad C#] -->|Render| HT[HTML element]
    CS -->|StateHasChanged| HT
    
    style CS fill:#F57C00,stroke:#FF9800,color:#fff
    style HT fill:#388E3C,stroke:#4CAF50,color:#fff
```

**Two-Way (edición):**

```razor
<!-- Blazor: Edición bidireccional -->
<InputText @bind-Value="formData.Nombre" />

<!-- Equivalente con evento oninput -->
<InputText @bind-Value="formData.Nombre" @bind-Value:event="oninput" />
```
...
%%{init: {'theme': 'dark', 'themeVariables': {'darkMode': true, 'background': '#1e1e1e'}}}%%
graph LR
    CS[Propiedad C#] <-->|@bind| HT[HTML Input]
    CS -->|StateHasChanged| HT
    HT -->|onchange/oninput| CS
...
```razor
<!-- One-Way: Solo muestra el nombre -->
<h3>@estudiante.NombreCompleto</h3>

<!-- Two-Way: Edita el nombre -->
<InputText @bind-Value="formData.Nombre" @bind-Value:event="oninput" />
...
@code {
    ...
    private EstudianteFormData formData = new();
    private string AppTitle => "Sistema de Gestión Académica";
}
```

---

### 11.1.2. Binding a Colecciones con List\<T\>

En Blazor se usa `List<T>` directamente con `@foreach`:

```razor
<table class="table">
    <thead>
        <tr><th>ID</th><th>Nombre</th><th>Email</th><th>Nota</th></tr>
    </thead>
    <tbody>
...
@code {
    ...
    private async Task CargarDatosAsync()
    {
        ...
        result.Match(
            ...
            {
                estudiantes = data.OfType<Estudiante>().ToList();
                estudiantesFiltrados = new List<Estudiante>(estudiantes);
            },
            onFailure: err => { /* manejar error */ }
        );
    }

    private void Filtrar(string texto)
    {
        estudiantesFiltrados = string.IsNullOrWhiteSpace(texto)
            ? new List<Estudiante>(estudiantes)
            : estudiantes.Where(e =>
...
### 11.1.3. Renderización de Tablas con @foreach
        ...
        @foreach (var est in estudiantesFiltrados)
        {
            <tr>
                ...
                <td>
                    @if (est.Imagen != null)
                    {
                        <img src="data:image/jpeg;base64,@Convert.ToBase64String(est.Imagen)" 
                             width="40" height="40" />
                    }
                </td>
                <td>
...
@code {
    ...
    private async Task EditarAsync(int id)
    {
        selectedEstudiante = estudiantesFiltrados.FirstOrDefault(e => e.Id == id);
        Navigation.NavigateTo($"/estudiantes/editar/{id}");
    }
...
### 11.1.4. Select con Enums

**Binding directo a enum:**

```razor
<select @bind="formData.Ciclo">
    @foreach (var ciclo in Enum.GetValues<Ciclo>())
    {
        <option value="@ciclo">@ObtenerDescripcion(ciclo)</option>
    }
</select>
```
...
```csharp
private string ObtenerDescripcion(Ciclo ciclo) => ciclo switch
{
...
// Para listas con descripción
private List<SelectOption<Ciclo>> CiclosDisponibles => new()
{
...
public record SelectOption<T>(T Value, string Label);
```

---

## 11.2. Reactividad con StateHasChanged

### 11.2.1. StateHasChanged en Blazor

Blazor usa `StateHasChanged()` para notificar al componente que debe re-renderizarse. Este método se llama automáticamente después de eventos del ciclo de vida y handlers de UI.
...
<p>@mensaje</p>
<button @onclick="ActualizarMensaje">Click</button>

@code {
    private string mensaje = "Hola";

    private async Task ActualizarMensaje()
    {
        mensaje = "Actualizado!";
        // StateHasChanged() se llama automáticamente después de @onclick
    }
}
```

**StateHasChanged manual para operaciones asíncronas:**
...
{
    isLoading = true;
    // StateHasChanged() NO se llama automáticamente aquí
    // porque no es un handler de evento de UI
...
**Propiedades calculadas con getter:**

```csharp
private List<Estudiante> estudiantes = new();

// Propiedad calculada (se evalúa en cada render)
public int TotalEstudiantes => estudiantes.Count;
public double NotaMedia => estudiantes.Any() ? estudiantes.Average(e => e.NotaMedia) : 0;
public int PorcentajeAprobados => estudiantes.Count > 0
    ? (int)(estudiantes.Count(e => e.NotaMedia >= 5) * 100.0 / estudiantes.Count)
    : 0;
```

### 11.2.2. Renderizado Condicional
...
### 11.2.3. Notificación en Cascada

**Escenario:** Cambiar `Ciclo` actualiza `AsignaturasDisponibles` y limpia `Notas`.
...
public class EstudianteFormData
{
    ...
    public double NotaMedia => Notas.Any() ? Notas.Average(n => n.Calificacion) : 0.0;
}
```
...
@code {
    ...
    private void OnCicloChanged(Ciclo? nuevoCiclo)
    {
        formData.Ciclo = nuevoCiclo;
        formData.Notas.Clear();

        if (nuevoCiclo.HasValue)
        {
            foreach (var asignatura in formData.AsignaturasDisponibles)
            {
                formData.Notas.Add(new NotaFormData
                {
...
%%{init: {'theme': 'dark', 'themeVariables': {'darkMode': true, 'background': '#1e1e1e'}}}%%
graph TD
    A[Usuario selecciona Ciclo] --> B[formData.Ciclo = DAW]
    B --> C[OnCicloChanged]
    C --> D[Limpiar Notas]
    C --> E[Crear notas para DAW]
    C --> F[StateHasChanged]
    
    F --> K[UI actualiza select]
    F --> L[UI renderiza nuevas notas]
    ...
    style F fill:#388E3C,stroke:#4CAF50,color:#fff
    style K fill:#388E3C,stroke:#4CAF50,color:#fff
    style L fill:#388E3C,stroke:#4CAF50,color:#fff
```

### 11.2.4. Diagrama de Secuencia: Flujo de Reactividad en Blazor
...
sequenceDiagram
    actor U as Usuario
    participant IP as InputText
    participant R as Blazor Renderer
    ...
    IP->>R: oninput event
    R->>C: searchText = "José"
    C->>C: FilterEstudiantes()
    C->>L: LINQ Where(...).ToList()
    L-->>C: List filtrada
...
## 11.3. Conversores en Blazor

En Blazor no existen los `IValueConverter` de WPF. En su lugar, se usan métodos C# en el code-behind o propiedades calculadas para transformar datos.

### 11.3.1. Mostrar/Ocultar con @if
...
<!-- Mostrar solo si isLoading = false -->
@if (!isLoading)
{
    <table class="table">
        <!-- datos -->
...
### 11.3.2. Convertir Enum a String

```csharp
// Método en code-behind
private string ObtenerDescripcionCiclo(Ciclo ciclo) => ciclo switch
{
    Ciclo.DAM => "Desarrollo de Aplicaciones Multiplataforma",
    Ciclo.DAW => "Desarrollo de Aplicaciones Web",
    Ciclo.ASIR => "Administración de Sistemas Informáticos en Red",
...
**Uso:**
```razor
<td>@ObtenerDescripcionCiclo(est.Ciclo)</td>
```

### 11.3.3. Mostrar Imagen desde Byte Array

```csharp
// Método en code-behind para generar data URI
private string? ObtenerImagenSrc(byte[]? imagenBytes)
{
...
@if (est.Imagen != null)
{
    <img src="@ObtenerImagenSrc(est.Imagen)" width="100" height="100" />
}
else
{
...
### 11.3.4. Convertidores Personalizados con Métodos C#

**Nota a Color:**

```csharp
private string ObtenerColorNota(double nota) => nota switch
{
    < 5.0 => "danger",     // Rojo
    >= 5.0 and < 7.0 => "warning",  // Naranja
    >= 7.0 and < 9.0 => "primary",  // Azul
    >= 9.0 => "success",   // Verde
    _ => "secondary"
};
```
...
**Uso:**
```razor
<td class="text-@ObtenerColorNota(est.NotaMedia)">
    @est.NotaMedia.ToString("N2")
</td>
```
...
```csharp
private string ObtenerIconoEstado(bool isDeleted) => isDeleted ? "❌" : "✅";
```

**Uso:**
```razor
<span>@ObtenerIconoEstado(est.IsDeleted)</span>
```

**Nombre completo (combinar propiedades):**

```razor
<td>@($"{est.Nombre} {est.Apellidos}")</td>
```

**Ventajas del enfoque Blazor:**
- Sin necesidad de clases `IValueConverter`
- Tipado fuerte con C# directo
- Sin registro global en recursos
- Fácil de testear (métodos puros)
- Menos código boilerplate

---

# 12. Búsqueda y Filtrado (Frontend)

## 12.1. Búsqueda en Tiempo Real

### 12.1.1. Búsqueda con @bind:event="oninput"

```razor
<input @bind="searchText" @bind:event="oninput" placeholder="🔍 Buscar por nombre, DNI o email..." />
```

**Code-behind:**

```csharp
public partial class BusquedaEstudiantes : ComponentBase
{
    [Inject] private IPersonasService Service { get; set; } = null!;
    ...
    private async Task FilterEstudiantesAsync()
    {
...
### 12.1.2. Filtrado con LINQ en Blazor

**Ventaja:** Filtrar en memoria sin recargar datos del servidor.
...
@code {
    ...
    {
        estudiantesFiltrados = string.IsNullOrWhiteSpace(searchText)
            ? new List<Estudiante>(estudiantes)
            : estudiantes.Where(e =>
                e.Nombre.Contains(searchText, StringComparison.OrdinalIgnoreCase) ||
                e.Apellidos.Contains(searchText, StringComparison.OrdinalIgnoreCase) ||
...
### 12.1.3. Filtros Combinados (AND/OR)
...
public partial class EstudiantesViewModel : ObservableObject
{
    ...
    private void FilterEstudiantes()
    {
        ...
        EstudiantesFiltrados = filtered.ToList();
    }
}
```
...
### 12.1.4. Debouncing para Performance

**Problema:** Filtrar en cada tecla puede ser costoso con muchos datos.

**Solución:** Esperar a que el usuario deje de escribir con CancellationToken.
...
@code {
    ...
    private async Task OnSearchTextChanged(string value)
    {
        ...
        try
        {
            await Task.Delay(DebounceMilliseconds, _debounceTokenSource.Token);
            await FilterEstudiantesAsync();
        }
        catch (TaskCanceledException)
        {
            // Búsqueda cancelada por nuevo cambio
        }
    }
...
### 12.1.5. Diagrama de Secuencia: Búsqueda y Filtrado (Async)
...
sequenceDiagram
    ...
    U->>IP: Escribe "J"
    IP->>C: searchText = "J"
    C->>Deb: Delay(300ms, token1)
    
    U->>IP: Escribe "Jo"
    IP->>C: searchText = "Jo"
    C->>Deb: Cancel(token1)
    C->>Deb: Delay(300ms, token2)
    
    U->>IP: Escribe "José"
    IP->>C: searchText = "José"
    C->>Deb: Cancel(token2)
    C->>Deb: Delay(300ms, token3)
    
    Note over U: Usuario deja de escribir
    
    Deb->>C: 300ms elapsed
    C->>C: LINQ Where(...).ToList()
    C->>C: StateHasChanged()
```

---

## 12.2. Ordenación Multicriterio

### 12.2.1. Ordenación con LINQ
...
### 12.2.2. Select para Criterio de Ordenación
...
### 12.2.3. Integración con TipoOrdenamiento Enum
...
**Componente con integración completa:**

```csharp
public partial class ListaEstudiantes : ComponentBase
{
    [Inject] private IPersonasService Service { get; set; } = null!;
...
# 13. Gestión de Estado Visual (Frontend)

## 13.1. Estados de Carga

### 13.1.1. IsLoading con Blazor
...
### 13.2.1. StatusMessage en Blazor
...
### 13.2.2. Colores por Estado (Success/Warning/Error)
...
### 13.2.3. Animaciones de Transición
...
### 13.2.4. Validación Visual en Campos
...
## 14.1. Flujo CRUD: Crear Estudiante (Async)

### 14.1.1. Diagrama de Secuencia Completo (Async/Await)

```mermaid
%%{init: {'theme': 'dark', 'themeVariables': {'darkMode': true, 'background': '#1e1e1e'}}}%%
sequenceDiagram
    actor U as Usuario
    participant P as Page (Blazor)
    participant CB as CodeBehind
    participant FD as FormData
    ...
    U->>P: Click Nuevo
    P->>CB: await NuevoAsync()
    
    CB->>FD: new FormData
    
    U->>CB: Completa campos
    CB->>FD: @bind actualiza propiedades
    Note over FD: Validación FluentValidation
    
    U->>CB: Click Seleccionar Imagen
    CB->>IS: await SaveImageAsync(path)
    
    IS->>IS: await ValidarExtension
    IS->>IS: await ValidarTamano
    IS->>IS: await ValidarDimensiones
    
    alt Validacion imagen OK
        IS-->>CB: Result.Success nombreArchivo
        CB->>FD: Imagen = nombreArchivo
    else Validacion imagen falla
        IS-->>CB: Result.Failure ImagenError
        CB->>U: Dialog.ShowError(error)
    end
    
    U->>CB: Click Guardar
    CB->>CB: Validar formulario
    
    alt Formulario válido
        CB->>PS: await CrearAsync(estudiante)
        
        PS->>VP: await ValidarPersonaAsync(estudiante)
        VP-->>PS: Result.Success
        
        PS->>VE: await ValidarEstudianteAsync(estudiante)
        VE-->>PS: Result.Success
        
        PS->>PS: Ensure DNI único
        PS->>PS: Ensure Email único
        
        PS->>R: await CreateAsync(estudiante)
        Note over R: async I/O (JSON/Dapper/EF)
        R-->>PS: Estudiante guardado
        
        PS->>C: Add(id, estudiante)
        
        PS-->>CB: Result.Success(estudiante)
        CB->>CB: Navigation.NavigateTo("/estudiantes")
    else Formulario inválido
        CB->>U: Muestra errores validación
    end
...
```csharp
// Blazor: Pages/Estudiantes/Lista.razor.cs
[Inject] private NavigationManager Nav { get; set; } = null!;

private async Task NuevoAsync()
{
    Nav.NavigateTo("/estudiantes/editar/0");
}
...
```csharp
// Blazor: Pages/Estudiantes/Editar.razor.cs
[Inject] private IPersonasService Service { get; set; } = null!;
[Inject] private IDialogService Dialog { get; set; } = null!;
[Inject] private NavigationManager Nav { get; set; } = null!;
...
protected override async Task OnInitializedAsync()
{
    if (Id > 0)
    {
        var result = await Service.ObtenerPorIdAsync(Id);
        result.Match(
            onSuccess: est => original = est as Estudiante,
            onFailure: err => Dialog.ShowError(err.Message)
        );
    }
}
...
private async Task GuardarAsync()
{
    ...
    var estudiante = formData.ToModel(Id);

    var result = Id == 0
        ? await Service.CrearAsync(estudiante)
        : await Service.ActualizarAsync(Id, estudiante);

    result.Match(
        onSuccess: _ =>
        {
            Dialog.ShowSuccessAsync("✅ Guardado correctamente");
            Nav.NavigateTo("/estudiantes");
        },
        onFailure: error =>
        {
            Dialog.ShowErrorAsync($"❌ {error.Message}");
        }
    );
}
...
1. `EstudiantesFiltrados` es `List<Estudiante>`
2. `@foreach` itera sobre la lista en cada render
3. `StateHasChanged()` re-renderiza el componente
4. Blazor actualiza el DOM automáticamente en cada render
...
### 13.3.3. Blazor Component Model (separación UI/lógica)
...
### 14.3.1. Validación en Frontend (FluentValidation/DataAnnotations) para UX
...
## 14.4. Gestión de Estado en Blazor Server

### 14.4.1. StateHasChanged y reactividad

**Lo aprendido:**
- Blazor necesita **notificaciones** (StateHasChanged) cuando el estado cambia para re-renderizar la UI.
- `INotifyPropertyChanged` es el mecanismo estándar: disparas `PropertyChanged` y WPF actualiza el binding.
- `StateHasChanged()` notifica a Blazor que debe re-renderizar el componente.

**Por qué es fundamental:**
- **Reactividad**: La UI se actualiza automáticamente cuando cambias `Nombre` en el componente.
...
### 14.4.2. Colecciones dinámicas en Blazor

**Lo aprendido:**
- `List<T>` se usa directamente en Blazor; al llamar `StateHasChanged()` el componente se re-renderiza.
- No se necesita `ObservableCollection<T>` porque Blazor re-renderiza todo el componente al cambiar el estado.
- Para listas grandes, se usa `Virtualize` para renderizado eficiente.
...
**Trade-off:**
- En Blazor no se necesita `ObservableCollection` porque `StateHasChanged()` re-renderiza todo el componente.

---

### 14.4.3. Filtrado en Blazor con LINQ

**Lo aprendido:**
- En Blazor se usa **LINQ** para filtrar y ordenar colecciones directamente.
- `Where()`, `OrderBy()`, `ToList()` crean nuevas colecciones filtradas.
- `StateHasChanged()` dispara la re-renderización con los datos filtrados.
...
### 14.4.4. Source Generators en Blazor

**Lo aprendido:**
- **Source Generators** generan código automáticamente en tiempo de compilación, también útiles en Blazor.
- Las propiedades auto-implementadas en el code-behind se actualizan con `StateHasChanged()`.
- Blazor no necesita `INotifyPropertyChanged` porque el RenderTree maneja las actualizaciones.
...
### 14.5.1. Inyección de Dependencias y Event Callbacks en Blazor

**Lo aprendido:**
- En Blazor, la comunicación entre componentes se hace mediante **parámetros** (`[Parameter]`) y **EventCallbacks** (`EventCallback<T>`).
- Los componentes padre pasan datos a hijos con parámetros. Los hijos notifican eventos al padre con `EventCallback`.
- Los **servicios DI** (Scoped por circuito) permiten compartir estado entre componentes del mismo circuito.

**Por qué EventCallbacks:**
- **Flujo unidireccional**: Los datos fluyen del padre al hijo, los eventos del hijo al padre.
- **Sin acoplamiento**: El hijo no conoce al padre, solo invoca el callback.
- **Reutilización**: El mismo componente puede ser usado por diferentes padres.

**Comparación Blazor vs WPF:**
- **WPF WeakReferenceMessenger**: Comunicación global pub/sub.
- **Blazor EventCallback**: Comunicación jerárquica padre → hijo.
- **Blazor DI Scoped**: Compartición de estado entre componentes del mismo circuito.
...
### 14.6.5. Caché Dual (búsquedas por ID y DNI)

**Lo aprendido:**
- Una caché por ID (`ICache<int, Persona>`) optimiza `GetById(id)`.
- Una segunda caché por DNI (`ICache<string, Persona>`) optimiza `GetByDni(dni)`.
- Ambas se invalidan juntas en Update/Delete y se actualizan juntas en Create.

**Por qué dos caches:**
- `GetById` y `GetByDni` son operaciones frecuentes e independientes.
- Un mismo objeto debe estar en ambas caches (por su ID y por su DNI).
- Invalidación coordinada: cuando se modifica un registro, invalidamos ambas caches.

**Implementación:**
- Registro: `AddSingleton<ICache<int, Persona>>` + `AddSingleton<ICache<string, Persona>>`
- PersonaService recibe ambas como dependencias.
- Cada método de búsqueda consulta su caché correspondiente.
...
### 14.10.5. Testing de Componentes Gráficos (LiveCharts2)

**Lo aprendido:**
- Los gráficos de **LiveCharts2** son componentes de UI (WPF) y no se testean unitariamente.
- La **lógica de preparación de datos** para gráficos sí es testeable en el ViewModel.
- Separación clara: **ViewModel** (testeable) → **View** (no testeable, solo verificable manualmente).

**Por qué no testar la UI de gráficos:**
- LiveCharts2 requiere un contexto visual (Window/UserControl) para renderizar.
- Los tests de UI (UI automation) son frágiles, lentos y costosos.
- El objetivo es testar la **lógica de transformación de datos**, no el renderizado.

**Qué se testa (GraficosViewModel):**

| Método testeado | Qué verifica |
|----------------|---------------|
| `GetNotasDistribution()` | Distribución en 4 categorías (suspensos, aprobados, notables, sobresalientes) |
| `GetCalificacionesData()` | Cálculo de notas medias por ciclo |
| `GetTasaAprobadosPorCiclo()` | Porcentajes de aprobados/suspensos |
| `GetEstudiantesPorCurso()` | Conteo de Primero vs Segundo |
| `GetEstudiantesPorEdad()` | Distribución por rangos de edad |
...
public void GetNotasDistribution_SinEstudiantes_DeberiaRetornarCeros() {
    ...
    serviceMock.Setup(s => s.GetEstudiantesOrderBy(...))
...
**Resultado:** 641 tests unitarios pasando, incluyendo 10 tests para GraficosViewModel.
