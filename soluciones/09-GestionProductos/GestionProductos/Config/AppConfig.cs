...
public class AppConfig
{
    ...
    public static string ConnectionString => Config.GetValue<string>("Database:ConnectionString") 
        ?? $"Data Source={Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "data", "productos.db")}";
    ...
    public static string DataFolder
    {
        get
        {
            // Obtener la ruta completa de la carpeta data
            var baseDir = AppDomain.CurrentDomain.BaseDirectory;
            var folder = Path.Combine(baseDir, "data");
