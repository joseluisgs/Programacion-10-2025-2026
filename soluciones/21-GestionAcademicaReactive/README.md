# 🎓 Sistema de Gestión Académica (Reactive) - Guía Completa para Estudiantes
...
- [6. Arquitectura Frontend (WPF con MVVM)](#6-arquitectura-frontend-wpf-con-mvvm)
  ...
  - [6.2. Views (Presentación)](#62-views-presentación)
    ...
    - [6.2.5. GraficosView (LiveCharts2)](#625-graficosview-livecharts2)
    - [6.2.6. Dialogs (EditWindows)](#626-dialogs-editwindows)
    - [6.2.7. Diagrama de Navegación entre Vistas](#627-diagrama-de-navegación-entre-vistas)
  - [6.3. ViewModels](#63-viewmodels)
    ...
    - [6.3.5. GraficosViewModel](#635-graficosviewmodel)
    - [6.3.6. EstudianteEditViewModel](#636-estudianteeditviewmodel)
    - [6.3.7. DocenteEditViewModel](#637-docenteeditviewmodel)
    - [6.3.8. ItemViewModels (Optimización Reactiva)](#638-itemviewmodels-optimización-reactiva)
    - [6.3.9. Diagrama de Clases: Jerarquía de ViewModels](#639-diagrama-de-clases-jerarquía-de-viewmodels)
...
- [8. Navegación y Comunicación (Frontend)](#8-navegación-y-comunicación-frontend)
  - [8.1. Sistema de Navegación](#81-sistema-de-navegación)
    - [8.1.1. Frame + Page](#811-frame--page-navegación-principal)
    - [8.1.2. MainWindow.xaml](#812-mainwindowxaml)
    - [8.1.3. Navegación en code-behind](#813-navegación-en-code-behind)
    - [8.1.4. ShowDialog()](#814-showdialog-diálogos-modales)
    - [8.1.5. Flujo de navegación](#815-flujo-de-navegación)
    - [8.1.6. Paso de Parámetros](#816-paso-de-parámetros-entre-vistas)
    - [8.1.7. Gestión de Modal Dialogs](#817-gestión-de-modal-dialogs)
    - [8.1.8. Callbacks con Action](#818-callbacks-con-action)
...
├── GestionAcademica.Front/      # UI WPF con MVVM
└── GestionAcademica.Test/       # Tests unitarios
```

**¿Por qué tres proyectos separados?**
...
| **Testabilidad** | La lógica de negocio se testa sin depender de WPF. Back y Test compilan sin necesidad de entorno Windows. |
...
| **Reutilización** | Un mismo Back puede alimentar múltiples Frontends (WPF actual + Blazor futuro + Mobile con MAUI). |
| **Clean Architecture** | Separa claramente responsabilidad: Datos → Dominio → Servicios → UI |

**Migración a otras tecnologías (ejemplo Blazor)**

1. **Mantener Back intacto** - Servicios, repositorios, validadores no cambian
2. **Crear nuevo proyecto Blazor** - Referenciar Back
3. **Implementar UI en Blazor** - Usar los mismos servicios del Back
4. **Reutilizar Tests** - Los tests del proyecto Test siguen funcionando

```
Futuro:
├── GestionAcademica.Back/       ← Sin cambios
├── GestionAcademica.Front/      ← WPF actual
├── GestionAcademica.Blazor/     ← Nuevo: Blazor WebAssembly
├── GestionAcademica.Maui/       ← Futuro: MAUI móvil
└── GestionAcademica.Test/       ← Sin cambios
```

**Gestión de Dependencias en Frontend**

El Frontend usa los servicios del Backend mediante inyección de dependencias:

- **Back**: Define `GestionAcademica.Infrastructure.DependenciesProvider` (servicios core)
- **Front**: Define `GestionAcademica.Infrastructure.FrontDependenciesProvider` (usa Back + ViewModels)
- **App.xaml.cs**: Llama a `FrontDependenciesProvider.BuildServiceProvider()`
...
### 6.2.5. GraficosView (LiveCharts2)

**Responsabilidad:** Visualización de gráficos estadísticos interactivos utilizando **LiveCharts2** (LiveChartsCore.SkiaSharpView.WPF v2.0.0).
...
|---------|------|-------------------|
| `NotasChart` | CartesianChart (barras) | Notas medias por ciclo |
| `DocentesChart` | CartesianChart (barras) | Docentes por ciclo |
| `EstudiantesChart` | CartesianChart (barras) | Estudiantes por ciclo |
| `DistributionChart` | PieChart (doughnut) | Distribución de calificaciones |
| `TasaAprobadosChart` | CartesianChart (stacked) | % aprobados/suspensos por ciclo |
| `CursosChart` | CartesianChart (horizontal bar) | Estudiantes por curso (Primero/Segundo) |
| `EdadChart` | CartesianChart (barras) | Distribución por edad |

**Componente en XAML:**

```xml
<UserControl x:Class="GestionAcademica.Views.Graficos.GraficosView"
    ...
    <Grid>
        ...
        <!-- Gráfico de notas medias por ciclo -->
        <lvc:CartesianChart Grid.Row="0" Grid.Column="0"
                             Series="{Binding NotasChartSeries}"
                             XAxes="{Binding NotasChartXAxes}"
                             YAxes="{Binding NotasChartYAxes}"
                             TooltipPosition="Top"/>

        <!-- Gráfico doughnut de distribución -->
        <lvc:PieChart Grid.Row="0" Grid.Column="1"
                      Series="{Binding DistributionChartSeries}"
                      InitialRotation="-90"/>
    </Grid>
</UserControl>
```

**Inicialización de gráficos en code-behind:**

```csharp
private void InitializeNotasChart() {
    var ciclos = _viewModel.GetCicloLabels();
    var medias = _viewModel.GetCalificacionesData();
    ...
    NotasChart.XAxes = new Axis[] {
        new Axis { Labels = ciclos, LabelsRotation = 15 }
    };
...
```csharp
// Tooltip con formato personalizado según tipo de datos
NotasChart.Tooltip = new DefaultTooltip {
    TextSize = 12,
    BackgroundPaint = new SolidColorPaint(SKColors.White),
    TextPaint = new SolidColorPaint(SKColor.Parse("#333333"))
};
```

**Características de LiveCharts2:**

- **Animaciones fluidas:** Transiciones suaves al cargar datos
- **Interactividad:** Tooltips, zoom, hover states
- **Personalización:** Colores, fuentes, ejes, labels
- **Variedad de gráficos:** Cartesian, Pie, Line, Stacked, etc.
- **Binding reactivo:** Actualización automática al cambiar datos

**Testing:** Los métodos del `GraficosViewModel` tienen **tests unitarios** que verifican:
- Cálculo de notas medias por ciclo
- Distribución de calificaciones (suspensos, aprobados, notables, sobresalientes)
- Distribución por edad
- Tasa de aprobados por ciclo
- Estudiantes por curso (Primero/Segundo)

---

### 6.2.6. Dialogs (EditWindows)
...
### 6.2.7. Diagrama de Navegación entre Vistas
...
### 6.3.5. GraficosViewModel

**Responsabilidad:** Generación de datos para visualizaciones estadísticas. Proporciona métodos que transforman los datos del servicio en formatos listos para LiveCharts2.

**Dependencias:**
- `IPersonasService`: Para obtener estudiantes y docentes
- `ILogger` (Serilog): Para logging de operaciones

**Propiedades observable:**
- `TotalEstudiantes`: Total de estudiantes en el sistema
- `TotalDocentes`: Total de docentes en el sistema
- `MediaNotas`: Nota media global de estudiantes
- `EstudiantesAprobados`: Cantidad de estudiantes con nota >= 5
- `EstudiantesSuspensos`: Cantidad de estudiantes con nota < 5
- `EstudiantesNotable`: Cantidad con nota 7-8.9
- `EstudiantesSobresaliente`: Cantidad con nota 9-10

**Métodos públicos:**

| Método | Retorno | Descripción |
|--------|---------|-------------|
| `GetCalificacionesData()` | `double[]` | Notas medias por ciclo |
| `GetCicloLabels()` | `string[]` | Nombres de ciclos (DAM, DAW, ASIR) |
| `GetNotasDistribution()` | `double[]` | Distribución: [suspensos, aprobados, notables, sobresalientes] |
| `GetDocentesPorCiclo()` | `(double[], string[])` | Docentes agrupados por ciclo |
| `GetEstudiantesCantidadPorCiclo()` | `(double[], string[])` | Estudiantes por ciclo |
| `GetEstudiantesPorEdad()` | `Dictionary<string, int>` | Distribución: menores 18, 18-25, mayores 25 |
| `GetTasaAprobadosPorCiclo()` | `(double[], double[], string[])` | Porcentaje aprobados/suspensos por ciclo |
| `GetEstudiantesPorCurso()` | `(int, int, string[])` | Estudiantes en Primero vs Segundo |
...
public GraficosViewModel(IPersonasService personasService) {
    _personasService = personasService;
    LoadStatistics(); // Carga inicial de datos
}

// LoadStatistics calcula:
// - TotalEstudiantes, TotalDocentes
// - MediaNotas (Average de todas las calificaciones)
// - EstudiantesAprobados, EstudiantesSuspensos
// - EstudiantesNotable, EstudiantesSobresaliente
```
...
public void GetNotasDistribution_DeberiaRetornar4Categorias() {
    ...
    var estudiantes = new List<Estudiante> {
        new() { Id = 1, Calificacion = 3.0 },  // Suspenso
        new() { Id = 2, Calificacion = 6.0 },  // Aprobado
        new() { Id = 3, Calificacion = 8.0 },  // Notable
        new() { Id = 4, Calificacion = 10.0 }  // Sobresaliente
    };
    ...
    resultado[0].Should().Be(1); // Suspensos
    resultado[1].Should().Be(1); // Aprobados
    resultado[2].Should().Be(1); // Notables
    resultado[3].Should().Be(1); // Sobresalientes
}

[Test]
public void GetTasaAprobadosPorCiclo_DeberiaRetornarPorcentajes() {
    // Verifica cálculo de tasa de aprobados por ciclo
    // Returns: (aprobados[], suspensos[], ciclos[])
}
```

**Integración con LiveCharts2:**

El ViewModel no conoce LiveCharts2. Proporciona solo datos puros. La vista (`GraficosView.xaml.cs`) consume estos métodos y crea las series de LiveCharts2:

```csharp
// En GraficosView.xaml.cs
private void InitializeDistributionChart() {
    var distribution = _viewModel.GetNotasDistribution();
    var total = distribution.Sum();
    
    var labels = distribution.Select((v, i) => {
        var pct = total > 0 ? v / total * 100 : 0;
        return $"{categorias[i]}: {(int)v} ({pct:F1}%)";
    }).ToArray();
    
    DistributionChart.Series = new ISeries[] {
        new PieSeries<double> { Values = new[] { distribution[0] }, Name = labels[0] },
        // ...
    };
}
```
...
### 6.3.7. DocenteEditViewModel

**Estructura similar a `EstudianteEditViewModel` pero para docentes.**

---

### 6.3.8. ItemViewModels (Optimización Reactiva)

Para maximizar el rendimiento y la fluidez de la interfaz, el sistema utiliza el patrón **ItemViewModel** para representar elementos en las listas (`ObservableCollection`).

**🚀 Ventajas del ItemViewModel:**
*   **Reactividad Granular:** Al heredar de `ObservableObject`, cualquier cambio en una propiedad (ej: Nota o Nombre) se refleja instantáneamente en la `DataGrid` sin redibujar la fila ni refrescar la colección completa.
*   **Actualizaciones Quirúrgicas:** Tras editar un registro, el sistema sincroniza solo el objeto visual existente, evitando llamadas costosas al servicio de carga.
*   **Separación UI/Dominio:** El modelo de dominio (`Estudiante`) permanece como un POCO inmutable, mientras que el `ItemViewModel` maneja la reactividad requerida por WPF.

**💎 ¿Qué obtenemos con esta solución?**

1.  **Experiencia de Usuario Premium**:
    *   **Adiós al "Flickering"**: Al no recargar la lista completa tras cada edición, la UI no parpadea ni se pierde el scroll o la fila seleccionada.
    *   **Estados en Tiempo Real**: Al borrar lógicamente a un usuario, este puede volverse semitransparente o itálico al instante (vía bindings a `IsDeleted`) sin que desaparezca de la vista si el filtro lo permite.
2.  **Rendimiento Técnico de Alto Nivel**:
    *   **Ahorro de Tráfico**: Solo se envía a la base de datos la petición de actualización. La lista visual se actualiza localmente con los datos del formulario que ya tenemos en memoria.
    *   **Escalabilidad**: El sistema puede manejar miles de registros en una `ObservableCollection` sin que las operaciones CRUD se degraden, ya que el impacto visual es O(1) (solo un elemento cambia).
3.  **Mantenibilidad**:
    *   **Código Limpio**: La lógica de "qué se ve" y "cómo se ve" queda en el `ItemViewModel`, manteniendo los servicios de backend enfocados únicamente en los datos puros.
...
### 6.3.9. Diagrama de Clases: Jerarquía de ViewModels
...
classDiagram
    ...
    class EstudiantesViewModel {
        -IPersonasService _personasService
        +ObservableCollection~EstudianteItemViewModel~ Estudiantes
        +EstudianteItemViewModel SelectedEstudiante
        +string SearchText
        +NuevoCommand()
        +EditarCommand()
        +EliminarCommand()
    ...
    class DocentesViewModel {
        -IPersonasService _personasService
        +ObservableCollection~DocenteItemViewModel~ Docentes
        +DocenteItemViewModel SelectedDocente
        +NuevoCommand()
        +EditarCommand()
    }
    
    class EstudianteItemViewModel {
        +double Calificacion
        +Ciclo Ciclo
        +Curso Curso
    }
    
    class DocenteItemViewModel {
        +int Experiencia
        +string Especialidad
        +Ciclo Ciclo
    }
    ...
    ObservableObject <|-- EstudianteItemViewModel
    ObservableObject <|-- DocenteItemViewModel
    
    EstudiantesViewModel o-- EstudianteItemViewModel : contiene*
    DocentesViewModel o-- DocenteItemViewModel : contiene*
...
La navegación permite cambiar entre diferentes vistas de la aplicación sin recargar la ventana principal.

### 8.1.1. Frame + Page (Navegación principal)
...
**MainWindow:**
```csharp
viewModel.OnNavigateRequested += page => MainFrame.Navigate(page);
```

**Convención de nombres:**

| ViewModel | Vista (UserControl) |
| ------------------------- | ------------------------------- |
| (no hay ViewModel)       | `EstudiantesView`              |
| (no hay ViewModel)       | `DashboardView`                 |

> 📝 **Nota**: Este proyecto NO usa NavigationService. Las vistas se crean directamente con `new VistaView()`.

### 8.1.2. MainWindow.xaml
...
<Grid>
    ...
    <StackPanel>
        ...
        <Button Content="Estudiantes" Command="{Binding NavigateToEstudiantesCommand}"/>
    </StackPanel>
...
### 8.1.3. Navegación en code-behind
...
### 8.1.4. ShowDialog() - Diálogos Modales

```csharp
// Ventana de detalles
new EstudianteDetailsWindow { Owner = MainWindow }.ShowDialog();

// Ventana de edición (con resultado)
if (new EstudianteEditWindow { DataContext = vm }.ShowDialog() == true) { ... }

//AcercaDeWindow
new AcercaDeWindow { Owner = this }.ShowDialog();
```

### 8.1.5. Flujo de navegación

```
Usuario → Botón → Command → Evento → MainFrame.Navigate(page) → Nueva Vista
```

### 8.1.6. Paso de Parámetros entre Vistas
...
### 8.1.7. Gestión de Modal Dialogs
...
### 8.1.8. Callbacks con Action<bool>
