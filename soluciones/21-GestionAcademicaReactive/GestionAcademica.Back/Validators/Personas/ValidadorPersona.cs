...
public static class ValidadorPersonaExtensions {
    ...
    public static bool IsValidName(this string name) {
        return !string.IsNullOrWhiteSpace(name) && name.Trim().Length >= 2 && name.Length <= 30;
    }

    public static bool IsValidSurnames(this string surnames) {
        return !string.IsNullOrWhiteSpace(surnames) && surnames.Trim().Length >= 2 && surnames.Length <= 50;
    }

    public static bool IsValidBirthDate(this DateTime date) {
        return date.Year >= 1900 && date <= DateTime.Today;
    }
