...
public abstract record ImageError(string Message) : DomainError(Message) {
    ...
    public sealed record FileSizeTooLarge(string FileName, long ActualSize, long MaxSize)
        : ImageError(
            $"El archivo {FileName} ({ActualSize / 1024 / 1024}MB) excede el tamaño máximo permitido ({MaxSize / 1024 / 1024}MB)");

    public sealed record DimensionsTooLarge(string FileName, int Width, int Height, int MaxWidth, int MaxHeight)
        : ImageError(
            $"La imagen {FileName} ({Width}x{Height}) excede las dimensiones máximas permitidas ({MaxWidth}x{MaxHeight})");
...
public static class ImageErrors {
    public static DomainError NotFound(string fileName) {
        return new ImageError.NotFound(fileName);
    }

    public static DomainError InvalidFormat(string extension) {
        return new ImageError.InvalidFormat(extension);
    }

    public static DomainError SaveError(string details) {
        return new ImageError.SaveError(details);
    }

    public static DomainError DeleteError(string details) {
        return new ImageError.DeleteError(details);
    }

    public static DomainError AccessError(string details) {
        return new ImageError.AccessError(details);
    }

    public static DomainError FileSizeTooLarge(string fileName, long actualSize, long maxSize) {
        return new ImageError.FileSizeTooLarge(fileName, actualSize, maxSize);
    }

    public static DomainError DimensionsTooLarge(string fileName, int width, int height, int maxWidth, int maxHeight) {
        return new ImageError.DimensionsTooLarge(fileName, width, height, maxWidth, maxHeight);
    }

    public static DomainError ValidationError(string details) {
        return new ImageError.ValidationError(details);
    }
}