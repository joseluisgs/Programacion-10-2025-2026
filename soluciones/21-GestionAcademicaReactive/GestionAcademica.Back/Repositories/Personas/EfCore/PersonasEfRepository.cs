...
public class PersonasEfRepository : IPersonasRepository {
    ...
    public PersonasEfRepository(AppDbContext context, bool dropData = false, bool seedData = false) {
        _context = context;
        ...
        if (seedData && !_context.Personas.Any()) {
    ...
    public IEnumerable<Persona> GetAll(int page = 1, int pageSize = 10, bool includeDeleted = true) {
        try {
            // AsNoTracking() mejora rendimiento en consultas de solo lectura porque no guarda las entidades en el ChangeTracker
            var query = includeDeleted
                ? _context.Personas.AsNoTracking()
                : _context.Personas.Where(p => !p.IsDeleted).AsNoTracking();
    ...
    public IEnumerable<Estudiante> GetEstudiantes(int page = 1, int pageSize = 10, bool includeDeleted = true) {
        try {
            // AsNoTracking() mejora rendimiento en consultas de solo lectura porque no guarda las entidades en el ChangeTracker
            var query = includeDeleted
                ? _context.Personas.Where(p => p.Tipo == "Estudiante").AsNoTracking()
                : _context.Personas.Where(p => p.Tipo == "Estudiante" && !p.IsDeleted).AsNoTracking();
    ...
    public IEnumerable<Docente> GetDocentes(int page = 1, int pageSize = 10, bool includeDeleted = true) {
        try {
            // AsNoTracking() mejora rendimiento en consultas de solo lectura porque no guarda las entidades en el ChangeTracker
            var query = includeDeleted
                ? _context.Personas.Where(p => p.Tipo == "Docente").AsNoTracking()
                : _context.Personas.Where(p => p.Tipo == "Docente" && !p.IsDeleted).AsNoTracking();
    ...
    public Persona? GetById(int id) {
        try {
            // AsNoTracking() mejora rendimiento en consultas de solo lectura porque no guarda las entidades en el ChangeTracker
            var entity = _context.Personas.AsNoTracking().FirstOrDefault(p => p.Id == id);
    ...
    public Persona? GetByDni(string dni) {
        try {
            // AsNoTracking() mejora rendimiento en consultas de solo lectura porque no guarda las entidades en el ChangeTracker
            var entity = _context.Personas.AsNoTracking().FirstOrDefault(p => p.Dni == dni);
            return entity.ToModel();
        }
        catch (Exception ex) {
    ...
    public Result<Persona, DomainError> Update(int id, Persona model) {
        ...
        if ((model.Dni ?? "") != (existingModel.Dni ?? "") &&
            _context.Personas.Any(p => p.Dni == (model.Dni ?? "") && p.Id != id))
            return Result.Failure<Persona, DomainError>(PersonaErrors.DniAlreadyExists(model.Dni ?? ""));
    ...
    public IEnumerable<Estudiante> GetEstudiantesOrderBy(string orden, int page = 1, int pageSize = 10, bool includeDeleted = true) {
        try {
            // AsNoTracking() mejora rendimiento en consultas de solo lectura porque no guarda las entidades en el ChangeTracker
            var query = includeDeleted
                ? _context.Personas.Where(p => p.Tipo == "Estudiante").AsNoTracking()
    ...
    public IEnumerable<Docente> GetDocentesOrderBy(string orden, int page = 1, int pageSize = 10, bool includeDeleted = true) {
        try {
            var query = includeDeleted
                ? _context.Personas.Where(p => p.Tipo == "Docente")
                : _context.Personas.Where(p => p.Tipo == "Docente" && !p.IsDeleted);
