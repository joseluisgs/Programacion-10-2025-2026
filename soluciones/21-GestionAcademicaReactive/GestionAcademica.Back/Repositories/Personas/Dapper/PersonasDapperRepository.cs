...
public class PersonasDapperRepository : IPersonasRepository {
    ...
    public IEnumerable<Persona> GetAll(int page = 1, int pageSize = 10, bool includeDeleted = true) {
        try {
            ...
            var entities = _connection
                .Query<PersonaEntity>(sql, new { PageSize = pageSize, Offset = (page - 1) * pageSize }).ToList();
    ...
    public IEnumerable<Estudiante> GetEstudiantesOrderBy(string orden, int page = 1, int pageSize = 10, bool includeDeleted = true) {
        ...
        return _connection.Query<PersonaEntity>(sql).ToModel().Cast<Estudiante>();
    }
    ...
    public IEnumerable<Docente> GetDocentesOrderBy(string orden, int page = 1, int pageSize = 10, bool includeDeleted = true) {
        ...
        var offset = (page - 1) * pageSize;
        var sql = $"SELECT * FROM Personas {where} ORDER BY {orderBy} LIMIT {pageSize} OFFSET {offset}";
        return _connection.Query<PersonaEntity>(sql).ToModel().Cast<Docente>();
    }
    ...
    private void EnsureTable(bool dropData) {
        ...
        if (dropData) _connection.Execute("DROP TABLE IF EXISTS Personas");
    ...
    private void Seed() {
        foreach (var p in PersonasFactory.Seed()) Create(p);
    }
}