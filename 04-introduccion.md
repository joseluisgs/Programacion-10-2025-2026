...
- [4.4. Controles Básicos de WPF](#44-controles-básicos-de-wpf)
  ...
  - [4.4.6. Menu y MenuItem](#446-menu-y-menuitem)
  - [4.4.6. Menu y MenuItem](#446-menu-y-menuitem)
    - [4.4.6.1. Estructura básica](#4461-estructura-básica)
    - [4.4.6.2. Aceleradores de teclado (InputGestureText)](#4462-aceleradores-de-teclado-inputgesturetext)
    - [4.4.6.3. KeyBinding - Ejecutar comandos con teclas](#4463-keybinding---ejecutar-comandos-con-teclas)
    - [4.4.6.4. Código C# - MVVM con Commands](#4464-código-c---mvvm-with-commands)
    - [4.4.6.5. Resumen: Aceleradores de teclado](#4465-resumen-aceleradores-de-teclado)
...
#### 4.4.6. Menu y MenuItem

Menú de aplicación con opciones y **aceleradores de teclado**.

##### 4.4.6.1. Estructura básica

```xml
<Menu>
    <MenuItem Header="_Archivo">
        <MenuItem Header="_Nuevo" Click="MenuNuevo_Click" />
        <MenuItem Header="_Abrir" Click="MenuAbrir_Click" />
        <Separator />
        <MenuItem Header="_Guardar" Click="MenuGuardar_Click" />
        <MenuItem Header="Guardar _como..." Click="MenuGuardarComo_Click" />
        <Separator />
        <MenuItem Header="_Salir" Click="MenuSalir_Click" />
    </MenuItem>
    <MenuItem Header="_Editar">
        <MenuItem Header="_Cortar" Click="MenuCortar_Click" />
        <MenuItem Header="_Copiar" Click="MenuCopiar_Click" />
        <MenuItem Header="_Pegar" Click="MenuPegar_Click" />
    </MenuItem>
    <MenuItem Header="_Ayuda">
        <MenuItem Header="_Acerca de" Click="MenuAcerca_Click" />
    </MenuItem>
</Menu>
```

> 💡 El guión bajo (`_`) antes de una letra crea el **mnemonic** (alt+letra para acceder).

##### 4.4.6.2. Aceleradores de teclado (InputGestureText)

Muestra atajos como Ctrl+S, Ctrl+N junto a las opciones de menú:

```xml
<Menu>
    <MenuItem Header="_Archivo">
        <MenuItem Header="_Nuevo" InputGestureText="Ctrl+N">
            <MenuItem.Click>
                <EventHandler EventName="Click" Handler="MenuNuevo_Click" />
            </MenuItem.Click>
        </MenuItem>
        <MenuItem Header="_Abrir" InputGestureText="Ctrl+O" />
        <Separator />
        <MenuItem Header="_Guardar" InputGestureText="Ctrl+S" />
        <MenuItem Header="Guardar _como..." InputGestureText="Ctrl+Shift+S" />
        <Separator />
        <MenuItem Header="_Salir" InputGestureText="Alt+F4" />
    </MenuItem>
</Menu>
```

##### 4.4.6.3. KeyBinding - Ejecutar comandos con teclas

Para que el atajo **realmente funcione**, usamos `InputBindings` con `KeyBinding`:

```xml
<Window x:Class="MiVentana"
        xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
        Title="Menú con Aceleradores">
    
    <!-- Definimos los KeyBindings a nivel de Window -->
    <Window.InputBindings>
        <!-- Ctrl+N para Nuevo -->
        <KeyBinding Key="N" Modifiers="Control" Command="{Binding ComandoNuevo}" />
        <!-- Ctrl+O para Abrir -->
        <KeyBinding Key="O" Modifiers="Control" Command="{Binding ComandoAbrir}" />
        <!-- Ctrl+S para Guardar -->
        <KeyBinding Key="S" Modifiers="Control" Command="{Binding ComandoGuardar}" />
        <!-- Ctrl+Shift+S para Guardar Como -->
        <KeyBinding Key="S" Modifiers="Control+Shift" Command="{Binding ComandoGuardarComo}" />
    </Window.InputBindings>
    
    <Menu>
        <MenuItem Header="_Archivo">
            <MenuItem Header="_Nuevo" Command="{Binding ComandoNuevo}" InputGestureText="Ctrl+N" />
            <MenuItem Header="_Abrir" Command="{Binding ComandoAbrir}" InputGestureText="Ctrl+O" />
            <Separator />
            <MenuItem Header="_Guardar" Command="{Binding ComandoGuardar}" InputGestureText="Ctrl+S" />
            <MenuItem Header="Guardar _como..." Command="{Binding ComandoGuardarComo}" InputGestureText="Ctrl+Shift+S" />
        </MenuItem>
    </Menu>
</Window>
```

##### 4.4.6.4. Código C# - MVVM con Commands

```csharp
// ViewModel con Commands
public partial class MainViewModel : ObservableObject
{
    ...
    {
        // Lógica para Nuevo
        System.Windows.MessageBox.Show("Nuevo documento");
    }
    ...
    private void Abrir()
    {
        ...
        {
            System.Windows.MessageBox.Show($"Abierto: {dialog.FileName}");
        }
    }
    ...
    {
        // Lógica para Guardar
        System.Windows.MessageBox.Show("Documento guardado");
    }
    
    [RelayCommand]
    private void GuardarComo()
    {
        // Lógica para Guardar Como
        var dialog = new SaveFileDialog();
        if (dialog.ShowDialog() == true)
        {
            System.Windows.MessageBox.Show($"Guardado como: {dialog.FileName}");
        }
    }
}
...
##### 4.4.6.5. Resumen: Aceleradores de teclado

| Elemento | Propósito |
|----------|-----------|
| `Header="_Archivo"` | Mnemonic: Alt+A accede al menú |
| `InputGestureText="Ctrl+S"` | Muestra "Ctrl+S" en el menú |
| `<KeyBinding Key="S" Modifiers="Control" />` |Hace que Ctrl+S **funcione** |
| `Command="{Binding ComandoGuardar}"` | Ejecuta la lógica del comando |

**Flujo completo:**
```
1. Usuario presiona Ctrl+S
2. Window.InputBindings detecta la tecla
3. Ejecuta ComandoGuardard (del ViewModel)
4. Se muestra elmnemonic (Ctrl+S) junto a "Guardar"
```
